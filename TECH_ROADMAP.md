...
>
> **当前进度：web-0 ✅ / web-1 ✅ / web-2 ✅ / web-3 ✅ / web-4 ✅ / web-5 ✅ / web-6 ✅（w6.3 拆成「归档页」`ec5fee8` 与「RSS 订阅」`6b5b9ec` 两个提交；另补了 w6.1d「后台分类可编辑」`ecc276d`）/ web-7 🔄 3.5/4（w7.3 已完成 `fa251b9`，w7.4 拆 `admin.vue` 进行中）。**
> 前端三件套每一批提交前都实测过一次（`eslint .` 0 问题 / `npm run test` 全绿 / `npm run build` 通过），
> 用例数逐批累加（w7.3 之后是 **26 个测试文件 / 470 个用例**）：
...
> `ec5fee8` / `6b5b9ec` / `ecc276d` 这三个提交落地后，我在**干净工作区**上又独立复跑了一次：
> `eslint .` 0 问题、**25 个文件 466 个用例全绿**、`npm run build` 成功
> —— 与提交信息里的自报数字一致（不是照抄提交信息）。
> `fa251b9`（w7.3）我复核的是**产物**：JS 2 483 252 字节 / CSS 299 419 字节（与它自报的改后值逐字节一致），
> 26 个测试文件与它自报的一致；**470 这个用例数我还没自己跑过**，等 w7.4 收尾时一起实测。
> ⚠️ 顺带记一处**它自己 README 里没对上的地方**：那张逐文件表是 **25 行 / 合计 462**，
> 缺的是 `test/useIdempotencyKey.nuxt.spec.ts`（8 个用例，462 + 8 = 470）——
> 已经连同证据交给它在这一批里补上（后端 README 前几次犯的是同一个毛病：总数对、明细漏一行）。
...
### web-7 · SEO 与性能（前端专属，1 天）—— 🔄 3.5/4 完成（w7.3 ✅，w7.4 进行中）
...
>
> **剩余**：w7.4（拆 `admin.vue`：`fa251b9` 之后它是 **2141 行**，注释里那句"1845 → 2138"已随之过期）
> —— 分类管理那一条已经在 w6.1d 里接完了
...
| **w7.4** | `拆分后台单文件组件` | `admin.vue` 已超过 1000 行，拆成 用户表格 / 文章表格 / 标签 / 评论 / 编辑器弹窗等组件（对应 D3） | 拆分后 `npm run test` 与手动回归全通过 |
