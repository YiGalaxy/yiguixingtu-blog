...
| **6.1b** | `新增文章打标签与按标签筛选` | `ArticleForm.tagIds`、`ArticleVO.tags`、`ArticleQuery.tagId`；`ArticleServiceImpl` 接线（create/update 时覆盖式写关联、列表与详情带上标签、按标签筛文章）；缓存失效 | 用例：打标签后详情/列表能带出标签、按标签筛选只返回该标签下的【已发布】文章、取消标签、草稿隔离、改标签让缓存失效 |
