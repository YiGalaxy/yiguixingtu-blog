...
>
> **当前进度：web-0 ✅ / web-1 ✅ / web-2 ✅ / web-3 ✅ / web-4 ✅ / web-5 ✅ / web-6 ✅（w6.3 待做）/ web-7 🔄 3/4（w7.3 / w7.4 待做）。**
> 前端三件套在 HEAD 上实测：`eslint .` 0 问题、**16 个测试文件 280 个用例全绿**、`npm run build` 通过
> （用例数一路是 53 → 92 → 121 → 132 → 167 → 189 → 205 → 214 → 219 → 223 → 243 → 280）。
...
### web-6 · 内容功能（与后端 6.x 对齐）—— 🔄 进行中

> 后端 6.1（标签 + 打标签与筛选）、6.1c（分类增删改）、6.2（评论与审核）、
> 6.4（归档）、6.5（RSS 数据）都已经完成并通过 283 个用例，所以这一批是"接线"性质的：
...
>   前端目前只做了只读列表。要做成可编辑只是"再接三个接口"，可以放在 w7.4 拆完
>   `admin.vue` 之后顺手做（那时的组件结构正好适合放一个分类表单）。
...
| **w6.3** | `新增归档页与RSS订阅` | 归档按年月分组、`/feed.xml`（后端 6.4 归档 + 6.5 RSS 数据都已就绪） | 用例：归档分组正确；feed.xml 是合法 XML 且含最近文章 |
