...
>
> **当前进度：web-0 ✅ / web-1 ✅ / web-2 ✅ / web-3 ✅ / web-4 ✅ / web-5 ✅ / web-6 ✅（w6.3 拆成「归档页」`ec5fee8` 与「RSS 订阅」`6b5b9ec` 两个提交；另补了 w6.1d「后台分类可编辑」`ecc276d`）/ web-7 🔄 3/4（w7.3、w7.4 待做）。**
> 前端三件套每一批提交前都实测过一次（`eslint .` 0 问题 / `npm run test` 全绿 / `npm run build` 通过），
> 用例数逐批累加：
> 53 → 92 → 121 → 132 → 167 → 189 → 205 → 214 → 219 → 223 → 243 → 280 → 404 → 419 → 448 → 466。
>
...
> 等 w7.3 / w7.4 收尾时再统一实测一次，把终值写回这一行。
...
>
> **剩余**：w7.3（Element Plus 按需引入）、w7.4（拆 `admin.vue`，顺便可以把
> 上面那条分类管理一起接上）
