...
### M2 · 缓存纵深（2 天，4 个提交）—— 🔄 2.3 已完成，其余暂停
...
>
> **2.1 / 2.2 / 2.4 仍暂停**：卡在"缓存写入偶发不落地"那个查不清的现象上，
> 改动暂存在 git stash 里（`git stash list` 可见）。2.1 那条"`RedisCache.clear()` 是异步的"
> 结论仍然有效，见下面「⚠️ 2.1 已暂停」那一段。
