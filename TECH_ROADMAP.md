...
| **M2** | 🔄 进行中（2.1 ✅ 列表缓存 · 2.3 ✅ 浏览量 · 2.4 ✅ 统计接口 / 2.2 详情缓存待做） | **A1 + A2**：缓存三件套 + 浏览量计数 | 「数据库与缓存」升级为掌握级；那条 `了解 Redis 高并发` **从"了解"行移出** |
...
### M2 · 缓存纵深（2 天，4 个提交）—— 🔄 2.1 ✅ · 2.3 ✅ · 2.4 ✅ / 2.2 待做

> **2.4 已完成**（本次提交）：`GET /article/stats`（文章数 / 总浏览量 / 分类数），
