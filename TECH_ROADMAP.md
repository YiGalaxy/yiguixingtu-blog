...
- 🔄 **方向调整（最重要）**：新增第 4 条原则 **「能用主流现成方案就不自研」** 与 **§20 自研 vs 主流：方案修订表**。
  ...
  **`@Cacheable(sync=true)`**（防击穿）、**Redis `SET NX`**（接口幂等，**不是 Redisson** —— Redisson 只在选型时评估过、
  最终没用，见 §20 与 5.4 的说明）、**`sensitive-word`**（敏感词，C 批可选）。
  ⚠️ **简历上不要出现 `Redisson`**：项目里没有这个依赖，写了会被一句"你在哪用的 Redisson"问穿。
...
| 3 | **`getPublishedDetail` 读接口里带写操作**：每次看详情都 `UPDATE view_count = view_count + 1` | 🔴 → ✅ **已解决** | `ArticleServiceImpl.java:114` → `:230` | 纯写放大 + 热点文章行锁竞争。注释里已经解释过"为什么用 SQL 自增而不是读改写"，方向是对的，只是这个写不该在读路径上。**已由提交 2.3 修复**：详情只做一次 Redis `INCR`，`ViewCountSyncTask` 每 5 分钟批量落库；返回的数字 = 库快照 + Redis 增量。实测：连续访问 3 次后数据库一行未改，14 秒后（把间隔临时设成 8 秒）库里 +3 且 Redis 清零 | ~~A2~~ **2.3** |
...
> 而是要加主流依赖：`resilience4j-spring-boot3`（限流/熔断）、`micrometer-tracing-bridge-brave`（链路追踪）。
> 这两样都是"配置文件 + 注解"就能用，不写一行基础设施代码。
> ⚠️ **2026-09-11 校正**：当时还列了第三个 `redisson-spring-boot-starter`（分布式锁），
> **最终没有引入** —— 2.2 的防击穿用 `@Cacheable(sync = true)`、5.4 的幂等用 Redis `SET NX`，
> 单实例部署下都不需要分布式锁（见 §20 第 3、6 行与 §E3 的说明）。
...
| 17 | **Redisson（分布式锁 / 布隆过滤器）** | 缓存击穿重建、接口幂等 | 手写 `SETNX` 要自己处理**锁续期、误删、可重入**；Redisson 是现成主流方案 | 2.2 · 5.4 | 分布式锁 | **❌ 评估后未采用**（2026-09-11 校正）：2.2 用 `@Cacheable(sync=true)`、5.4 用 Redis `SET NX`，单实例部署下够用；**简历上不要写 Redisson** |
| 18 | **sensitive-word（敏感词过滤库）** | 评论内容过滤 | DFA 前缀树**不自研**，用成熟库（自带分词、跳过、白名单） | C 批 | 敏感词过滤 | M6 后（可选） |

**这张表怎么用**

- **做一项、勾一项**：每完成对应提交，就在本表打勾，并同步简历（见 §8 简历同步规则）。
- **分类（2026-09-11 校正，原来只写了 1–16 三档、把 17/18 漏在外面）**：
  · **1–13 = 确定要做**（M1–M5 已在 §16 拆成原子提交）—— **全部已完成**；
  · **14–16 = 有余力再说**（C 批可选：RabbitMQ / ES+IK / Caffeine）—— **都没做**；
  · **17 = 评估后未采用**（Redisson：单实例部署下用 `@Cacheable(sync=true)` 与 Redis `SET NX` 就够）；
  · **18 = C 批可选**（`sensitive-word` 敏感词过滤）—— **没做**。
- 简历上**只写真正做完的那些**（也就是 1–13）。挑进简历的具体是哪几项，见下面「简历口径」。
...
| **技能栏 · 数据库与缓存** | `Redis` 从"缓存（Hash + TTL、主动失效）"升级为 **"缓存穿透 / 击穿 / 雪崩"**，并加 **`接口幂等（SETNX）`** —— ⚠️ **不要写 `Redisson 分布式锁`**（项目没引入 Redisson） | 对应 2.1 / 2.2 / 5.4 |
...
> **✅ 2026-09-11 更新：部署这条路已经在本机按生产形态跑通了（四容器 + 空库 + SSR）**
>   实测清单见后端 README「部署 → 这份清单在本机演练过一遍」，要点：
>     · 空库启动 → Flyway **6 个迁移**全部执行、7 张表建好
>     · 四个容器（mysql / redis / backend / **frontend SSR**）全部 healthy
>     · 只有 `127.0.0.1:8092` 与 `127.0.0.1:3010` 对外映射，**mysql/redis 一个端口都没映射**
>     · 前端容器通过服务名访问后端：首页 SSR HTML 含真实文章、文章页含评论、
>       标签是可爬的内链；`/sitemap.xml`、`/robots.txt`、`/feed.xml` 都是运行时路由
>     · 评论链路（游客发 → 待审核不可见 → 通过 → 可见）、审计留痕、上传落卷、限流 429 全部实测过
>
>   **⚠️ 部署时最容易踩的两个"不报错但很贵"的点**
>     ① **镜像构建会下载整个 Maven 依赖树**（Dockerfile 里的 `dependency:go-offline`）。
>        实测直连 Maven Central：**25 分钟没结束**（`-q` 吞掉了下载日志，看起来像卡死）；
>        换阿里云镜像后 **7.8 分钟**。Dockerfile 已默认写入阿里云镜像，
>        可用 `--build-arg MAVEN_MIRROR_URL=` 关掉或换成别的。
>     ② **`PUBLIC_SITE_URL` 必须填对**：canonical / sitemap / feed 三处都用它拼。
>        填错不会报错，只会让搜索引擎把权重算到别的域名上 —— 所以它用了 `${VAR:?}`，
>        没配直接起不来（宁可起不来，也不要带着错域名上线）。
...
- ~~**辅助：Redisson `RLock`** 按业务 key（`idem:{userId}:{接口}:{参数摘要}`，TTL 几秒）加锁~~
- **⚠️ 上面两条是原始设想，最终【都没有采用】**（2026-09-11 校正，以代码为准）：
  实现是 **Redis `SET NX` 占位 + 结果缓存**（`common/idempotency/IdempotencyService`，用
  Spring Data Redis 的 `setIfAbsent(key, value, Duration)`），**不用业务唯一索引、也没有引入 Redisson**。
  理由：唯一索引要往每张业务表加列并处理"失败后如何重试"，而本项目要防的是**同一个请求被重复提交**
  （双击发布 / 网络重试），一个带 TTL 的幂等键就够了；`SET NX` 一条命令天然原子，
  不需要分布式锁（没有跨实例部署，也就没有锁续期/可重入这些问题）。**不自研 `@Idempotent` 切面**这条没变
...
| **M3** | ✅ 完成（3.1 多环境 · 3.2 图片上传 · 3.3 Dockerfile+四容器 · 3.4 部署文档 · 3.5 管理员引导） | **E1 + B4 + B5 + B6**：多环境 → 图片上传 → 上线 → CD。⚠️ **存储方案已变更**：最终是"存部署服务器本地磁盘 + 具名卷持久化"，**OSS 实现与依赖已彻底删除**（见 §B4 变更记录与后端 README「文件上传」） | 技能栏加 `Docker Compose` / `Nginx`；结尾写「**阿里云 ECS**」（**不要再写 OSS**）；过一遍 §8「简历同步规则」第 1 条的不可见注释核对门。⚠️ 徽章「已上线」要等**真的部署完**再算数 —— 本机按生产形态完整演练过（见后端 README「部署」），但演练 ≠ 上线 |
| **M4** | ✅ 完成（4.1 ✅ 限流 · 4.2 ✅ 操作审计） | **A3 + A4**：Nginx + Resilience4j 限流、Spring 事件驱动的操作审计 | 技能栏加 `Resilience4j`（限流与熔断降级）；「切面与事务」那条**保持原样** —— `@Transactional` / `@PreAuthorize` 就是最主流的用法，不需要改。⚠️ **不要写 `Redisson`**：5.4 的幂等是用 **Redis 的 `SET NX`**（`setIfAbsent` + 结果缓存）做的，项目里**没有引入 Redisson**，写了会被一句"你在哪用的 Redisson"问穿（这条已同步到简历的核对注释里） |
| **M5** | ✅ 完成（5.1 ✅ 5.2 ✅ 5.3 ✅ 5.4 ✅ 5.7 ✅ 5.8 ✅ 5.9 ✅ / **5.6 完成一半**（已做掉最安全的那一半：地址不存在 → 真 404、方法用错 → 真 405；剩下"业务错误也统一成真实状态码"是**破坏性改动**，要前后端同一时间窗一起改，见 §5.6 说明）/ **5.5 可选未做**（登录验证码）） | **D + E**：traceId、索引与压测、指标、幂等、登出失效、安全头、错误码 | 面试纵深：**这些是"做过才答得出"的细节**。⚠️ `Redis Lua 限流`／自研切面／自建线程池**都不要写**（见 §20 自研 vs 主流） |
| **M6** | ✅ 完成（6.1a ✅ 标签 / 6.1b ✅ 打标签与筛选 / 6.1c ✅ 分类增删改 / 6.2 ✅ 评论与审核 / **6.4 ✅ 归档接口** / **6.5 ✅ RSS 数据接口** / **6.3 可选未做**（RabbitMQ）） | **C / F**：标签评论、搜索、消息队列、前端站点 | 标签、评论、分类后台管理、归档、RSS 都已做完；**6.3（RabbitMQ）与敏感词过滤仍是可选、未做** —— 所以简历上不要出现 RabbitMQ / ES 这类"项目里没有"的词 |

**顺序理由**：B1/B2 只是给测试和 CI 加壳、**不改业务代码**，风险最低，却立刻把**当时那 76 个用例**从"只能自己跑"变成"任何人都能验证"（用例数后来一路涨到 403，见 README），所以排在最前面；A 批改的是现有业务代码，收益最快但风险略高，紧随其后；E1/B5 负责把东西真正交付出去；D/E 其余是**在 A/B 建好的东西上加纵深**（traceId 的价值要靠 A4 的异步日志才体现）；C 需要新业务模块，放最后。
...
> ✅ **2026-09-11：这一节是当初的「范围征询」，答案已经是"全都做了"。**
> 逐项的真实状态请看 §9（后端里程碑 M0–M6）与 §18（前端提交清单 web-0 ~ web-7），
> 这里只作为**历史记录**保留 —— 下面那些勾是按结果补上的，不是当时的决定。
> （当初这份征询里还**没列出 web-4 / web-6**，它们后来也做了：web-4 限流处理、web-6 标签/评论/归档/RSS/分类管理。）

**A. 实施范围（挑一个或多个）** —— 实际结果：**A + B + D + E + C/F 全都做了**

- [x] **第一步（最稳，推荐先做这个）**：**M0 + M1** = G（文档对齐）+ B1/B2（Testcontainers + CI）+ B3（Flyway）—— 已完成
- [x] **最小组合**：A1 + A2 + B1 + B2 + B5 + G —— 已完成（而且是超集）
- [x] **完整批次 A + B**：缓存/切面/工程化/交付 —— 已完成
- [x] **再加 D（性能与可观测）**：+2 天，多出 EXPLAIN、traceId、压测数据 —— 已完成
- [x] **再加 E（安全与配置）**：+2–3 天，多出多环境、登出失效、幂等、权限矩阵 —— 已完成
- [x] **C / F**：业务补全与前端站点 —— 已完成（标签/评论/归档/RSS/分类管理 + 前端整站）

**A2. 前端范围（`yiguixingtu-web`，详见 §17–§19）**

- [x] **web-0 仓库抢救（半天，最紧急）**：把已经写好的 2200 行按功能补进仓库 —— 已完成（`32e5eae` / `491e0c5` / `6713354`）
- [x] **web-1 前端工程化**：Vitest 测试环境 + CI + API 地址环境变量化 —— 已完成
- [x] **web-2 前台补齐**：分类筛选、统计接口、去掉硬编码假数据 —— 已完成
- [x] **web-3 上线准备**：封面上传组件 ✅、静态资源（改由 Nginx 托管，**不上 OSS**）✅、Dockerfile ✅、README ✅
- [x] **web-5 安全体验**：登出链路、记住密码明文问题、traceId 提示、防连点 —— 已完成
- [x] **web-7 SEO 与性能**：首页 SSR、meta/OG、sitemap、按需引入 —— 已完成（w7.4 还额外拆了 `admin.vue`）

**B. 需要你提供的信息（没有就先占位）** —— 2026-09-11 校正后的**真实待办**

- [ ] ECS 地域/规格/公网 IP、备案号 —— **仍然需要**（你说过"到时候要部署到阿里云服务器上"，
      域名与证书都已申请好，具体信息等部署前给）
- [x] ~~OSS bucket / endpoint / 地域~~ —— **不再需要**：存储最终定为"服务器本地磁盘 + 具名卷"，
      OSS 实现与依赖已删除（见 §B4 变更记录与后端 README「文件上传」）
- [x] 前端形态：**SSG 静态**还是 **SSR Node** —— **已定：SSR Node 容器**（见 §17.3 的 E6 与 §18 的 w3.3）。
      这也意味着 `sitemap.xml` / `robots.txt` / `feed.xml` 必须是**运行时路由**、由 Nginx 转发给前端容器，
      不能配成静态文件（后端 README 的 Nginx 章节已写明）
- [x] 站点域名 —— 已申请（`www.yigalaxy.xin`），canonical / sitemap / robots / feed 的域名由
      `PUBLIC_SITE_URL` 一处控制（上线核对清单第 14 条守这件事）
...
>
> ⚠️ **本节是"原始计划"，不是进度表 —— 不要在这里追进度。**
>   实际做了什么、哪些改了方案、踩了什么坑，都记在对应里程碑的**进度块**里：
>     · 后端 M4 → §9 的 M4 进度块（4.1 / 4.2）
>     · 后端 M6 → §M6 表格（6.1a / 6.1b / 6.1c / 6.2 / 6.4 / 6.5，以及未做的 6.3）
>     · 前端 → §18 各 web-x 小节
>   保留这份原计划的价值在于**对照**：比如 6.1 当初是"一个提交"，实际拆成了三个
>   （标签模块 / 文章打标签 / 分类增删改）——为什么拆、拆完各自验证了什么，
>   在 §M6 里；而"当初是怎么想的"在这里。两者都留着才看得出演变。
...
### M2 · 缓存纵深（2 天，4 个提交）—— ✅ **全部完成**（2.1 ✅ · 2.2 ✅ · 2.3 ✅ · 2.4 ✅）

> **2.2 已完成**（本次提交）：文章详情缓存 + 防击穿，`ArticleDetailCacheTest` 11 个用例。
>
>   · **缓存边界划在"库里的那份数据"**：详情返回的浏览量 = 库快照 + Redis 增量，
>     而增量**每次访问都会变**。把最终结果整个缓存起来的话，命中缓存的请求
>     就不会再执行 INCR —— **浏览量会永远停在那个值上**。
>     页面照常打开、数字照常显示，只是不再增长，这种 bug 极难被发现。
>     所以 INCR 与合并都留在缓存外面，只有"读库"那一段进缓存
>   · **可缓存的那段单独放一个 Bean（PublishedArticleCache）**：
>     Spring Cache 靠代理生效，同类内部调用（this.load()）不走代理，注解等于没写
>   · **防击穿用 `sync = true`，不自己写 SETNX**：那正是写入器内部在做的事
>     （锁超时、异常释放、抢不到时重试都处理好了）。自己写只会多几个能写错的地方，
>     而错了的表现是"偶发多查一次库"，几乎不可能被发现
>   · **防击穿是被真数出来的**，不是嘴上说的：用 `@MockitoSpyBean` 给 ArticleMapper
>     套一层只记账的替身，**12 个线程同时打同一篇未缓存文章，selectById 只被调 1 次**
>   · 测试上的一个坑：并发要靠多线程，而别的线程**看不到当前测试事务里未提交的那一行**。
>     所以那条用例特意关掉测试事务（`@Transactional(propagation = NOT_SUPPORTED)`）并自己清理数据
>
> **⚠️ 全量测试抓出了一个真 bug（这次交叉影响的典型例子）**
>   加完详情缓存后，`ArticleViewCountTest` 两条用例变红，暴露出两个问题：
>   ① **INCR 排在了"是不是草稿"的校验前面** → 拿 id 探测草稿也会留下浏览痕迹。
>      改成"先 load（顺带校验并抛 404）→ 再 INCR → 再合并"。
>      （原来的顺序是对的，是我重排时弄错了；这条用例就是为它存在的）
>   ② **浏览量落库后，详情里的快照变旧 → 数字往回跳**：
>      一篇文章被看 3 次（库 0 + 增量 3 = 显示 3）→ 落库（库变 3、增量清零）→
>      再有人看：旧快照 0 + 新增量 1 = **显示 1**，从 3 掉到 1。
>   ②的修法：**详情单独一个版本号**。文章被写 → 两个版本号一起推进；
>   浏览量落库 → 只推进详情那个（列表里 view_count 是原样展示、不做合并，
>   只会滞后不会回跳，不该被每 5 分钟一次的无谓失效拖累）。
>   → 这条**单看缓存功能是想不到的**，靠的是"每次提交都跑全量"这条纪律。
>
> **2.4 已完成**（上一个提交）：`GET /article/stats`（文章数 / 总浏览量 / 分类数），
> `ArticleStatsTest` 8 个用例。
>   · 一条 SQL 同时算 COUNT 与 SUM（两个聚合扫的是同一批行，不该分两次查）
>   · 口径只算已发布：草稿不计入（否则首页说 50 篇、游客只数得出 30 篇）
>   · ⚠️ 手写 `@Select` 必须自己写 `deleted = 0` —— `@TableLogic` 只对
>     MyBatis-Plus 自己生成的 SQL 生效。漏了的话删掉的文章会被永远算进首页数字，
>     而且不会有任何报错。专门有一条用例盯着这件事
>   · 缓存 60 秒（列表缓存是 5 分钟）：因为"总浏览量"是**异步落库**的，
>     落库不推进缓存版本号，所以这个数字天生滞后，TTL 得短一点
>   · 复用列表缓存那套版本号做失效 —— 两者失效时机完全一致（都是"文章被写"），
>     共用之后发文只需 INCR 一次，两个缓存一起失效
>   · 实测（真实服务）：接口返回 `{articleCount:2, viewCount:28, categoryCount:3}`，
>     与直接查库的 `2 / 28 / 3` 完全一致
>
> **2.1 已完成**（上一个提交）：`RedisConfig` + `ArticleCacheVersion` + 列表缓存 + 防穿透/防雪崩，
> `ArticleCacheTest` 13 个用例、连跑 4 次全绿。
>
> **⚠️ 那个卡了很久的"缓存写入偶发不落地"是【假故障】—— 完整结论，别再重查**
>
>   之前记录的两个现象：① 空结果（total=0）根本没被写进 Redis；
>   ② 非空结果的用例大部分通过但每次失败的不一样（"造 8 条缓存数到 7 条"）。
>   结论：**两个都是读证据的方式错了，put 一直是正常的。**
>
>   决定性证据来自 `redis-cli MONITOR` 抓到的命令流（这次终于按上次记的建议做了）：
>
...
>     —— **空结果被正常缓存了**，"现象①"不成立
>   · put 发出后，接口方法就返回了；测试紧接着用【另一条连接】发 KEYS，
>     两条连接的命令到达 Redis 的先后顺序本来就没有保证 ——
>     KEYS 抢先被处理，就数到 0，于是"每次失败的不一样"完全取决于时序
>   · 也就是说：**是测试断言了一条系统并不承诺的时序**，
>     而不是代码有 bug。生产上最坏后果只是"缓存写入晚几百微秒"，
>     下次请求多查一次库，没有任何正确性问题
>
>   修法（在测试里，不在生产代码里）：
>     · 加 `awaitCacheKeys(期望条数, 超时)` 做有上限的轮询，
>       断言的对象从"写命令有没有在方法返回前到 Redis"
>       改成"缓存最终有没有写进去"——后者才是任何缓存实现都承诺的语义
>     · 顺带把主断言换成**行为断言**（第二次调用命中缓存 → 返回旧结果），
>       它比"数 Redis key"更贴近用户能看到的现象，也不依赖任何时序
>   教训值得单独记一句：**遇到偶发失败，先问"我这个断言依赖了什么保证？那个保证存在吗？"**
>
> **2.1 顺带做掉的事**
>   · **防穿透**：既然确认了空结果能正常缓存，就把它的 TTL 单独设成 30 秒
>     （`TtlFunction` 按内容判 IPage 是否为空）。之前因为"现象①"一直没敢做
>   · **不再是 `BatchStrategies.keys()`**：原方案要跟 `clear()` 的异步语义较劲才选了
>     KEYS 策略（会阻塞 Redis）。版本号方案压根不用 clear，改回默认写入器，
>     生产上少一个隐患
>   · 补了一条**已知边界的用例**：绕过应用直接改库时缓存不会失效，
>     最多等 TTL 自愈。把它钉成用例，将来谁加了 CDC/触发器会立刻看到红灯
>
> **2.3 已完成**（提交 `bf1646c`）：浏览量 Redis 计数 + 定时批量落库。
> 它不依赖列表缓存，所以能独立做完 —— 而它正好是 §0.2 里最后一个 🔴 真缺口。
> 至此四个真缺口（#1 登出、#3 写放大、#6 跨域、#9 测试不可移植、#13 空库无管理员）**全部关闭**。


| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **2.1** | ✅ `新增RedisConfig与文章列表缓存，处理缓存穿透与雪崩` | `config/RedisConfig.java`（Jackson + `JavaTimeModule`）、`@EnableCaching`、`pagePublished` 加 `@Cacheable`（key = 版本号 + 归一化查询条件）、**版本号失效**（`ArticleCacheVersion`）、空结果 30 秒防穿透、TTL 抖动防雪崩 | `ArticleCacheTest` 13 个用例：第二次走缓存、四个写操作都失效、TTL 区间、空结果也缓存、key 归一化 —— **已完成，见上方进度块** |
| **2.2** | ✅ `新增文章详情缓存与互斥重建，保证更新后缓存一致性` | `PublishedArticleCache`（单独 Bean，让 @Cacheable 代理生效）+ `@Cacheable(sync = true)` 做**防击穿**（不自己写 SETNX）；`viewCounter.increment` 与合并留在缓存外；**详情单独一个版本号**（浏览量落库时只推它） | 并发打热点 key **只回源一次**（12 线程 → selectById 仅 1 次，用 @MockitoSpyBean 真数过）；编辑后详情立刻更新；下架即 404；手动 DEL 后自愈重建；**浏览量不被缓存冻住** —— **已完成，见上方进度块** |
...
| **2.4** | ✅ `新增站点统计接口，供首页与后台概览展示全站数据` | `GET /article/stats`：已发布文章数 / `SUM(view_count)` / 分类数（**为前端 B1、D1 补的接口**，见 §17.4）；走缓存 + 60 秒 TTL | `ArticleStatsTest` 8 个用例：口径只算已发布、逻辑删除不计入、空库不报错、走缓存、写操作让缓存失效 —— **已完成，见上方进度块** |
...
> ⚠️ 4.x 做完再回来**升级简历**（§8 第 3 条）：技能栏加 `Resilience4j`（**只加这一个** ——
> ⚠️ **不要顺手加 `Redisson`**：5.4 的幂等是用 Redis `SET NX` 做的，项目里没有 Redisson 依赖）；
> 那条「切面与事务」**保持原样不要改**（`@Transactional` / `@PreAuthorize` 就是主流用法），
> **全篇都不要出现"自研切面/自定义注解"这类词**（见 §20）。

### M5 · 可观测与安全（2–3 天，9 个提交）

> ### ✅ 进度：M5 已完成（9 项里 **8 项做完**，剩 1 项是可选）
> 5.1 ✅ · 5.2 ✅ · 5.3 ✅ · 5.4 ✅ · **5.6 ✅ 一半**（已做掉最安全的那一半：地址不存在 → 真 404、
> 方法用错 → 真 405；剩"业务错误也统一成真实状态码"是**破坏性改动**，要前后端同一时间窗一起改，
> 见下方 5.6 的说明）· 5.7 ✅ · 5.8 ✅ · 5.9 ✅ · **5.5 未做**（登录验证码，**计划里当时标的就是"可选"**）
>
> ⇒ 所以这一节的"9 个提交"实际落地为 **8 项（其中 5.6 只做了最安全的一半）+ 1 项可选未做**，
> 与 §9 里程碑表里 M5 那行的口径一致（那行也曾经过期，已在同一次校正里改掉）。
>
> **5.4 已完成**（本次提交）：`Idempotency-Key` 幂等键 + 前端防连点，两个仓库各一个提交。
>   · 后端：`IdempotencyService`（Redis `SET NX` 占位 → 执行 → 存结果）+ `AdminArticleController.create`
>     三步写在 Controller 里（幂等是【接口层】语义，Service 不该知道幂等键长什么样）
>   · 前端：`useIdempotencyKey`（一次提交动作一个键）+ `saveArticle` 的连点拦截
>   · 实测（真实服务、非 MockMvc）：
>       带同一个键发两次 → 两次都返回 `data: 131340`（**同一个 id**），库里 **1 篇**
>       不带键发两次   → 库里 **2 篇**（对照组，证明测试本身有效）
>   · 几个刻意的决定都写进了 README 的「接口幂等」章节：
>       键在【打开弹窗时】生成而不是每次点保存（否则点两下就是两个键，照样写两篇）；
>       结果 TTL 24 小时、占位 TTL 60 秒（差两个数量级，原因是"处理中"是瞬时状态）；
>       失败必须释放占位（否则用户重试永远得到"处理中"，一个无法自救的失败）；
>       **Redis 挂了选择放行**（幂等是保护层，不该因为它自己挂了让用户发不出文章）
>       —— 并注明"扣款下单"这类场景要反过来选拒绝
>   · 为什么不做 DB 唯一索引兜底：它需要一条业务唯一性规则（"同作者不能同名"），
>     而用户完全可能真想写两篇同名的 —— 用业务约束实现技术目的会误伤正常场景
>
> **5.6 做了最安全的一半**（同一提交）
>   · 改了什么：地址不存在 → **真 404**；HTTP 方法用错 → **真 405**
>     （原来这两种都被兜底处理器转成 HTTP 200 + code 500）
>   · 为什么只改这两个：它们属于"**请求本身有问题**"，前端永远不会主动触发，
>     所以对前端零影响；而中间所有软件（Nginx 日志、监控告警、负载均衡）
>     都只看状态码，全站错误报 200 意味着**监控里错误率永远是 0%**
>   · 业务错误（密码错、参数错）**保持 200 + body.code 不变**，
>     因为那是业务语义的一部分，改了前端 19 个接口的错误分支都要重新回归
>   · 顺带把一个测试从"绕着写"改回正着写：5.7 那条
>     `otherActuatorEndpoints_shouldNotBeExposed` 原本因为拿不到 404，
>     只能断言"响应体里没有 env 数据"；现在直接断言 404，
>     **这本身就是这次改动的收益 —— 状态码能表达的东西不用再去解析内容**
>   · 剩下"业务错误也统一"的部分仍留在 5.6，需要前后端在同一个时间窗内一起改
>
> **5.8 已完成**（上一个提交）：连接池参数 + 压测对比，而且压测**压出了一个真问题**。
>
> 压测方法：`wrk` 跑在容器里打本机后端（Windows 上不用装任何东西）：
>   `docker run --rm skandyla/wrk -t4 -c50 -d20s --latency http://host.docker.internal:8082/article/page`
> 数据：往开发库塞 10 万行（8 万已发布），跑完删掉（按固定标题前缀精确删除）。
>
> **压出来的关键发现：分页的 COUNT 才是瓶颈，不是"取 10 行数据"**
>   V2 之后数据查询已经 0.09ms，但接口只有 39 req/s、平均 255ms ——
>   这不合常理，于是把一次分页请求拆开量：
>     · COUNT(*) WHERE deleted=0 AND status=1  → **74.1ms（占 99.8%）**
>     · 数据查询（走 idx_status_top_create）  → 0.09ms
>     · 分类名查询                            → 0.05ms
>   原因：idx_status_create 里没有 deleted 这一列，而每条 SQL 都被逻辑删除插件
>   加上 deleted = 0，于是 8 万条【每条都要回表】确认一次。
>   修法：V3 加覆盖索引 idx_deleted_status(deleted, status)，COUNT 变成
>   Covering index lookup（不回表）→ 21.4ms（3.5 倍）。
>   实测接口：**38.94 → 607/569 req/s，平均延迟 255.62 → 19.9/25.4ms**（约 15 倍）。
>   → 这条比"加了个索引"值钱得多：它说明**直觉（数据查询慢）是错的，
>     得把请求拆开量才能找到真正的瓶颈**。
>
> **连接池：量过之后才知道默认值该不该改**
>   池 10 → 623/666/657 req/s，P50 68~72ms；池 20 → 872/872 req/s，P50 50~52ms。
>   池大一点确实更好（+34% 吞吐），但池大小该跟着核数走：
>   HikariCP 官方公式是"核数 × 2 + 磁盘数"，本机容器里 32 核、线上 ECS 大概 2~4 核，
>   两者算出来差一个数量级。所以做成 `DB_POOL_SIZE` 可配置，默认 10，不写死。
>
> **⚠️ 一个必须说清楚的教训：P99 不能只看单次压测**
>   同一个配置（池 10 / 50 并发）跑了三次，P99 分别是 135ms / 516ms / 107ms；
>   池 20 两次是 521ms / 77ms。20 秒的压测里一次 GC 或 Docker Desktop 抖动
>   就足以污染尾延迟。所以 README 那张表里**只拿 QPS 与 P50 下结论，
>   P99 如实列出并标明不可靠** —— 把不可靠的数字当结论比不给数字更糟。
>
>
...
>   · 老索引 `idx_status_create` **必须留**：它撑的是"按发布时间排序"那条路径
>     （实测 0.068ms），新的 `(status, is_top, create_time)` 是最左前缀断了、
>     替不了它 —— 测试里专门有一条断言防"想当然地清理重复索引"
>   · 两个候选方案量过之后**否掉**了：加 `deleted` 反而更慢（0.11 → 0.30ms，
>     type 从 ref 退化 range）；加 `category_id` 只快 0.05ms 却要多维护一棵 B+ 树
>   · 慢查询日志：官方默认 `slow_query_log=OFF` + `long_query_time=10s`，
>     等于没有记录 —— 上面那条 183ms 的 SQL 在默认配置下永远查不出来。
>     现在两套 compose 都开了，阈值 0.1s
>   · ⚠️ 踩到一个很值得记的坑：**MySQL 会忽略权限为 777 的配置文件**
>     （`World-writable config file ... is ignored`），而 Windows bind mount
...
| **5.2** | ✅ `新增登出接口的Token失效机制` | Redis 黑名单（按 `jti`），登录时写入 `jti`、登出按剩余有效期拉黑，过滤器每次解析后查一次；前端登出后旧 token 立即不可用 | 登出后旧 token 调 `/auth/me` → **401**；未登出的 token 不受影响 —— **已完成（`LogoutTokenTest` 11 个用例）** |
...
| **5.4** | ✅ `新增基于Redis幂等键的接口防重复提交` | Redis `SET NX` 占位 + 结果缓存（**不自研 `@Idempotent` 切面**，也不用业务唯一索引）；前端 `useIdempotencyKey` + 连点拦截 | 同键两次只产生一篇文章且返回同一 id；不同键各自创建；不带键保持旧行为；失败后能重试 —— **已完成，见上方进度块** |
| **5.5** | `新增登录验证码与失败次数锁定`（可选） | Redis 存验证码 + 失败计数 | 验证码错误被拒；失败 5 次后锁定 |
| **5.6** | 🔶 已完成一半 `统一异常响应的HTTP状态码` | 已做：地址不存在 → 真 404、方法用错 → 真 405（这两类前端不会主动触发，改了对前端零影响）。**未做**：业务错误也统一成真实状态码 —— 那是破坏性改动，需前后端同一时间窗一起改 | 新增两条用例同时断言状态码与 body.code；业务错误用例保持原样 —— **见上方进度块** |
...
| **5.8** | ✅ `新增数据库连接池调优与压测对比数据` | HikariCP 显式参数（`maximum-pool-size` 走 `DB_POOL_SIZE` / `minimum-idle` / `connection-timeout=3s` / `max-lifetime=30m`）；用 `wrk` 压测并记录对比；**顺带压出 V3 覆盖索引** | 用例不受影响；README 有压测对比表；连接池参数有依据（不是抄数字）—— **已完成，见上方进度块** |
...
### M6 · 业务补全（按需 —— 用户已确认"标签和评论要做"）

> **拆分说明**：6.1 最后拆成了两个提交（标签模块本身 / 文章打标签与按标签筛选），
> 因为它们各自都能独立跑绿、独立回滚，放一个提交里就是"一次提交干了两件事"（违反 R3）。

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **6.1a** | ✅ `新增标签模块与后台标签管理` | `V5` 建 `tag` + `article_tag`；Tag 实体/Mapper/Service/Controller；前台 `GET /tag/list`（带已发布文章数、走缓存）、后台 CRUD；标签的增删改进操作审计 | `TagTest` 14 个用例：前台标签云与文章数口径（草稿不计）、增删改、**物理删除**（原生 SQL 数物理行）、删标签连带清关联、名字查重与首尾空格、权限、缓存命中与失效 —— **已完成** |
| **6.1b** | ✅ `新增文章打标签与按标签筛选` | `ArticleForm.tagIds`、`ArticleVO.tags`、`ArticleQuery.tagId`；`ArticleServiceImpl` 接线（create/update 覆盖式写关联、列表与详情带标签、按标签筛文章）；`PublishedArticleCache` 也把标签放进缓存（它只在改标签时变，与"每次访问都变"的浏览量不同） | `ArticleTagTest` 12 个用例：覆盖式语义、**校验先于写入**（失败不动原有标签）、去重、草稿不泄漏、标签不存在返回空页、列表每篇都带标签、删文章清关联、打标签后标签云计数立刻 +1 —— **已完成** |
| **6.1c** | ✅ `新增分类的后台增删改` | `AdminCategoryController` + `CategoryForm`；删除时检查"还有几篇文章在用"；逻辑删除沿用"改名释放唯一索引"；分类列表也接入同一个缓存版本号 | `CategoryAdminTest` 11 个用例：改名后前台立刻生效、重名与空格、**有文章时拒绝删除**、**删掉后同名能重建**、**文章逻辑删除后分类就能删**（守"手写 COUNT 要自己加 deleted=0"）、权限 —— **已完成** |
| **6.4** | ✅ `新增按年月分组的文章归档接口` | `GET /article/archive`（公开）：只含已发布、按年月分组、500 篇上限、走文章版本号缓存；**在 Java 里分组**（SQL 分组拿不到每组内的列表，用 GROUP_CONCAT 再拆更别扭） | `ArticleArchiveTest` 7 个用例：只含已发布、分组正确、月份与月内都倒序、空数据、匿名可访问、缓存命中与失效、key 带版本号且有 TTL —— **已完成** |
| **6.5** | ✅ `新增RSS订阅源的数据接口` | `GET /article/rss`（公开）：最近 20 篇已发布文章**含正文**、一条查询取回（而不是让前端拿 id 逐篇查）、走文章版本号缓存；**XML 由前端拼**（域名/feed 标题只有前端知道，与 sitemap 同一分工；Markdown 转 HTML 也交给前端渲染器） | `ArticleRssTest` 7 个用例：倒序、只含已发布、带正文、上限 20 且挤掉最早那篇、匿名可访问、缓存命中与失效、key 带版本号与 TTL —— **已完成** |
| **6.2** | ✅ `新增评论模块与管理员审核` | `V6` 建 `comment` 表；游客可评论 + 三态审核（0 待审核 / 1 已通过 / 2 已拒绝）；前台状态写死为"已通过"、公开 VO 不含邮箱与 IP（后台另用 `AdminCommentVO`）；入库前 HTML 转义；发表接口限流 20 次/分钟 | `CommentTest` 15 个用例：游客可发但默认待审核、传 `status=0` 也拿不到待审核内容、审核通过才可见、草稿不能评论、XSS 转义、公开响应体不含邮箱/IP、后台带敏感字段与文章标题、审核参数校验、逻辑删除、权限、限流 429、正序分页 —— **已完成** |
...
| `POST /upload?type=audio` | 后台**音乐曲目**上传 mp3（与图片同一接口，按 `type` 分流两套规则） | **新增** | F6（见 §21.5） |
| `GET /music/list` + `/admin/music/**` | 音乐页的曲目列表与后台曲目管理（此前曲目写死在前端） | **新增** | F6（见 §21.5） |
...
>
> **当前进度：web-0 ✅ / web-1 ✅ / web-2 ✅ / web-3 ✅ / web-4 ✅ / web-5 ✅ / web-6 ✅（w6.3 拆成「归档页」`ec5fee8` 与「RSS 订阅」`6b5b9ec` 两个提交；另补了 w6.1d「后台分类可编辑」`ecc276d`）/ web-7 ✅ 4/4（w7.1 `389544d` / w7.2 `28640b7` / w7.3 `fa251b9` / w7.4 `f4d4fbc`）—— **前端 web-0 ~ web-7 到此全部完成**。**
> 前端三件套每一批提交前都实测过一次（`eslint .` 0 问题 / `npm run test` 全绿 / `npm run build` 通过），
> 用例数逐批累加（w7.4 之后是 **26 个测试文件 / 470 个用例**）：
> 53 → 92 → 121 → 132 → 167 → 189 → 205 → 214 → 219 → 223 → 243 → 280 → 404 → 419 → 448 → 466 → 470。
>
> ⏳ **这里原来写死的"16 个测试文件 280 个用例"早就过期了**（那是 web-7 完成 3/4 时的数字，
> 后面 web-6 的标签、评论、归档、RSS 四批都没有回写）。所以上面改成**逐批累加的序列** ——
> 每一批的当次实测值与联调证据都写在**对应提交信息的【测试】【联调】两节**里，逐提交可查。
> `ec5fee8` / `6b5b9ec` / `ecc276d` 这三个提交落地后，我在**干净工作区**上独立复跑过一次：
> `eslint .` 0 问题、**25 个文件 466 个用例全绿**、`npm run build` 成功 —— 与提交信息里的自报数字一致。
> `fa251b9`（w7.3）我复核的是**产物**：JS 2 483 252 字节 / CSS 299 419 字节（与它自报的改后值逐字节一致）。
> `f4d4fbc`（w7.4）我先把**行数**核了：9 个文件里 8 个与自报完全一致，`ArticleEditDialog.vue`
> 实测 394 而它写 386（已让它改）。
> **✅ 终态三件套与体积我已独立跑过一遍（前端仓库空闲时、在 `f4d4fbc` 这个已提交状态上）**：
> `npm run lint` 无输出（0 问题）、**`npm run test` → `Test Files 26 passed (26)` / `Tests 470 passed (470)`**、
> `npm run build` 成功；`.output/public/_nuxt` 下 **JS 132 文件 / 2 487 246 字节**、
> **CSS 6 文件 / 299 404 字节**，后台那份 **`admin.*.css` = 151 757 字节** ——
> 四个数字与提交信息里的自报值**逐字节一致**（所以前面那句"自报值"现在可以改成实测值了）。
> ⚠️ 另记一处它 README 里还没对上的地方：那张逐文件表是 **25 行 / 合计 462**，
> 缺的是 `test/useIdempotencyKey.nuxt.spec.ts`（8 个用例，462 + 8 = 470）——
> 已经连同证据交给它补（后端 README 前几次犯的是同一个毛病：总数对、明细漏一行）。
...
>
> **已知的"不合规历史提交"（刻意不重写）**：`688b1f4` 与 `a5a921b` 这两个
> 是最早那批抢救进来的提交，**没有 `Signed-off-by` trailer**（R4 是后来才定的）。
> 处理方式是"在后面补纠正性的提交"，而不是改写历史 —— 重写别人可能已经看过的
> 历史提交，收益只是让审计脚本好看一点，代价却是 hash 全变。
> 所以做提交规范核查时，**这两个是白名单**（后端 55 个提交全部合规、
> 前端 28 个里只有这两个例外）。
...
### web-2 · 前台功能补齐（1–2 天，与后端 2.x 对齐）—— ✅ 已完成

> 已完成三个提交：w2.1 `5ef9bab`、w2.2 `a1686e7`、w2.3 `4f6f47f`。
> 前端用例 **53 → 92 → 121 → 132**（10 个测试文件），`npm run lint` 0 问题，`npm run build` 通过。
>
> **几个值得记下来的点（都是踩过才知道的）**
>   · `mockNuxtImport('useRouter')` 会让**整个测试文件起不来**：Nuxt 内部插件
>     `navigation-repaint` 也调用 `useRouter()`，拿到假 router 后调 `router.beforeResolve` 就抛错。
>     解法是把"读 query / 写 query"做成**注入点**（`createArticleFilter({ querySource, replaceUrl })`），
>     生产注入真 route/router，测试注入 ref + spy —— 顺带把纯逻辑变成了可单测的
>   · **防抖第一版写错了位置**：只防抖了"写地址栏"，请求仍跟着输入框走，
>     等于每敲一个字照样发一次 `/article/page`（后端这接口还有 300 次/分钟限流）。
>     拆成 `keywordInput`（即时）与 `keyword`（生效中）之后才对
>   · `router.replace()` 之后只 `flushPromises()` 地址栏还是旧值 —— 要过导航守卫里的一次
>     `requestAnimationFrame`，测试里得再等一个宏任务
>   · 假内容断言要用 `wrapper.text()` 而不是 `wrapper.html()`：happy-dom 下 html() 会带上
>     HTML 注释，而源码里留着"为什么删掉"的中文注释，用 html() 会因注释而误判
>   · 保留项：公告跑马灯（属于站点文案而不是伪装成接口数据）保留；
>     音乐卡片保留真实音源、去掉编造的曲名与上一首/下一首

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w2.1** | ✅ `新增首页分类筛选，并把搜索条件同步到地址栏` | 分类筛选 UI（现在数据拉了没用）、关键词与分类写入 query、加输入防抖（对应 B2 / B3） | 用例：URL 变化 → 请求参数正确；刷新后筛选状态保持 —— **已完成** |
| **w2.2** | ✅ `对接站点统计接口，修正首页与后台概览的数字口径` | 用 `GET /article/stats` 替换前端本地求和；**后台概览改为进入时主动加载**（对应 B1 / D1） | 用例：统计接口返回 → 渲染正确数字；接口失败降级不崩 —— **已完成** |
| **w2.3** | ✅ `移除首页硬编码的假数据与随之而来的死代码` | 删掉假留言面板、硬编码"1 人正在看"；RSS / GitHub / 邮箱按钮接真实地址或移除（对应 B4 / B5 / B6） | 用例：无后端数据时不出现任何假内容 —— **已完成** |

### web-3 · 上线准备（1–2 天，与后端 3.x 对齐）—— ✅ 4/4 完成

> 已完成：w3.1 后台上传封面组件 → `POST /upload`（提交 `5616535`）、
> w3.3 前端 Dockerfile 与生产构建配置（提交 `38f9385`）、
> w3.4 前端 README 的部署与环境变量章节（同上）、
> **w3.2 媒体文件移出构建产物（提交 `81eccf9`）**。
>
> **w3.2 实测（把"体积降下来"这句话变成数字）**
>   · `.output/public`：**17.24 MB → 3.51 MB**（剩下的 3.35 MB 是 `_nuxt` 里的 JS/CSS bundle）
>   · `.output` 整体：**29.28 MB → 15.56 MB**
>   · ⚠️ 口径纠正：Nitro 打印的 `Σ Total size: 12.7 MB` 是**服务端 bundle**的大小，
>     这次改动前后都是 12.7 MB —— 之前把"12.7MB 是大文件撑起来的"说反了，
>     被撑起来的是 `public/`（所以下面 w3.2 那一行也按实测改了）
>   · 做法：文件挪到仓库根的 `static-media/`（仍留在 git 里保证可复现），
>     前端只有一处拼 URL（`mediaUrl()`，前缀可由 `NUXT_PUBLIC_MEDIA_BASE` 覆盖），
>     `/media/**` 在**生产由 Nginx 直接读磁盘**、在**开发由一个仅 dev 生效的 Nitro 路由**提供
>     （否则本地首页就是一块坏掉的背景）；生产构建里那个路由被编译成纯 404、
>     `node:fs` / `node:path` 的 import 全被摇掉 —— 线上不存在任何"能从磁盘读文件"的路径
>   · dev 实测：`/media/bg-music.mp3` → 200 且字节数与文件一致、`curl -r 0-9` → **206** +
>     `Content-Range: bytes 0-9/2001172`；`..` / `%2e%2e%2f` / `.env` / `.exe` 一律 404，越界 Range → 416
>   · 后端一侧（本仓库）：README 部署章节补了 Nginx 的 `location /media/`
>     （`alias /var/www/media/`、`expires 7d`、**刻意不加 limit_req** ——
>     音视频是 Range 分段拉的，套限流会让播放中途卡住）、上传步骤与上线核对第 12 条
>
> **⚠️ 2026-09-10 方案调整：不再上 OSS，静态资源放服务器本地**
>   原本 w3.2 计划把视频/音频迁到 OSS + CDN。但既然封面图已经改成
>   存服务器磁盘（见后端 README「文件上传」章节的取舍说明），
>   静态资源也一并放本地更一致、也更简单：单台 ECS 上，
>   **Nginx 直接托管静态文件比绕一圈 OSS 更快**（少一次外网往返），
>   而且不用配第二个阿里云服务。
>   什么时候再考虑 OSS/CDN：带宽成为瓶颈、或者要上多台机器的时候。

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w3.1** | `新增后台封面图上传组件，对接OSS上传接口` | 替换 `artForm.cover` 的纯文本输入框（对应 E7） | 用例：上传成功后回填 URL；失败给出提示；类型/大小校验 |
| **w3.2** | ✅ `把背景视频与音乐移出构建产物，改由Nginx直接托管` | `.output/public` 17.24 MB → 3.51 MB（对应 E5）。**方案已调整：不上 OSS，放服务器本地由 Nginx 托管** | 产物体积明显下降 + Nginx 能直接返回该文件 —— **已完成，实测见上方** |
...
> w4.1 已完成（提交 `396d25e`）。
> **关键判断：后端有两种"429"，必须分开提示** —— 它们的 `body.code` 都是 429，
> 只能靠 **HTTP 状态码**区分：
>   · 真限流（Resilience4j）：HTTP **429**（`$fetch` 直接抛异常）→「请求过于频繁，请稍后再试」
>   · 幂等键命中"正在处理中"：HTTP **200** + `code=429` →「请求正在处理中，请勿重复提交」
>   混成一句的后果不是"不好看"，而是把用户往错的方向指：明明点了两下却被告知"稍后再试"，
>   他会以为第一次没成功，再点一下。原来的实现里 429 落进了兜底分支 → 显示"网络异常"。
>
> 冷却的拦截点放在 `useAuth.login()` 里而不是按钮的 `disabled`：
> **`disabled` 挡不住回车提交**，只有"发请求之前那一层"才拦得住。
>
> ⚠️ 顺带查出一个真实放大问题（已记入待办，本轮未改）：
> **ofetch 默认的 `retryStatusCodes` 里包含 429 与 500** —— GET 请求被限流时会自动重试一次，
> 也就是"越被限流越加倍打后端"，正好和限流的目的相反。

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w4.1** | ✅ `区分两种 429（限流 / 重复提交），并给登录按钮加冷却` | `app/utils/apiError.ts` 集中判断与文案；429 单独提示"请求过于频繁"（不再是"网络异常"）；登录按钮 10 秒冷却（对应后端 4.1） | 用例：HTTP 429 → 走 429 分支而非通用错误 —— **已完成** |

### web-5 · 安全与体验（1 天，与后端 5.x 对齐）—— ✅ 完成

> w5.1（`de3d48c`）、w5.4（`7734d1f`）之前已完成；
> 本轮补上 **w5.2（`3dc8db9`）** 与 **w5.3（`3556c08`）**。
>
> **w5.2 的关键点：只改代码是不够的**
>   老用户浏览器里那个带着明文密码的 cookie 不会自己消失，所以挂载时读一次，
>   发现带 `username` 之外的字段就**立刻改写**成只含用户名的值（`restore()` 的 migrated 分支）；
>   新值用"构造"而不是"从旧对象删字段"产出 —— 构造只依赖新格式有什么，写出去的不可能带别的。
>   兼容了对象 / JSON 字符串 / 纯字符串三种历史形态：只认一种的话，另一种形态的老 cookie 会继续躺着。
>
> **w5.3 的关键点：头名是 `X-Trace-Id`，不是 `X-B3-TraceId`**
>   后者是跨进程传播用的；回给浏览器的是后端 `TraceResponseHeaderFilter` 写的自定义头
>   （CORS 也已 `addExposedHeader("X-Trace-Id")`，否则浏览器读不到）。
>   读法上还有一处坑：`$fetch` 正常返回只给 body，而"接口报错"里一大半是
>   **HTTP 200 + code≠200**，所以必须用 ofetch 的 `onResponse` / `onResponseError` 钩子拿响应头，
>   只在 catch 里读会漏掉这一类。没有这个头时连括号一起省（不显示空"追踪号："）。

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w5.1** | `修复登出链路：调用后端登出接口并清理用户状态` | 调 `POST /auth/logout` + 清 `user` state；删掉指向不存在 `/login` 的死代码（对应 A1 / A3） | 用例：登出后 `user` 为空、token 被清；接口被调用一次 |
| **w5.2** | ✅ `把「记住密码」改成只记用户名，并清掉带着明文密码的老 cookie` | 只记用户名；老 cookie 主动改写（对应 A2） | 用例：勾选后 cookie 里**不含密码** —— **已完成** |
| **w5.3** | ✅ `错误提示带上后端追踪号 X-Trace-Id，方便联合排查` | 响应头 `X-Trace-Id` → 错误提示里带号（对应后端 5.1） | 用例：响应头有 traceId 时提示文案包含它 —— **已完成** |
| **w5.4** | `新增写操作的幂等保护与loading态` | 发布 / 删除按钮防重复点击（对应后端 5.4） | 用例：连点两次只发一次请求 |

### web-6 · 内容功能（与后端 6.x 对齐）—— ✅ 已完成（w6.1 / w6.1b / w6.1c / w6.1d / w6.2 / w6.2b / w6.3）

> 后端 6.1（标签 + 打标签与筛选）、6.1c（分类增删改）、6.2（评论与审核）、
> 6.4（归档）、6.5（RSS 数据）都已经完成（后端全量 `verify` 实测 285 个用例全绿），所以这一批是"接线"性质的：
> 前端把已经存在的接口接到界面上，不需要再动后端（除了一处：见下面的 w6.1 备注）。
>
> **已完成**
>   · w6.1 / w6.1b（提交 `1e48289`）：文章卡片与详情展示标签、点标签按 `tagId` 筛选、
>     **tagId 并进了原有的 `useArticleFilter`**（不是另起一套同步逻辑 ——
>     两套"谁写地址栏"的逻辑一定会打架，典型表现是"点标签之后分类高亮莫名消失"）、
>     后台文章弹窗支持多选标签（覆盖式提交，不选 = 空数组 = 清空）
>   · w6.1c（提交 `e559910`）：后台「分类 / 标签」占位菜单换成两个真实页面。
>     标签是完整 CRUD；**分类做成只读** —— 后端其实已经有写接口了，
>     但这一批不接分类的编辑，所以选择"展示真实数据 + 说明为什么改不了"，
>     而不是摆一个"该模块开发中"的假占位（占位等于把已经有的东西说成没有）
>   · w6.2 / w6.2b（评论组件 + 后台审核）：见下方进度块
>   · w6.3（**拆成两个提交** `ec5fee8` 归档页 / `6b5b9ec` RSS 订阅）：
>     · 归档页 `/archive`：按年月分组平铺（**不用折叠** —— 折叠后浏览器 Ctrl+F 搜不到收起
>       面板里的文字，而归档页恰恰是"我记得标题里有个词"的地方）；数据走
>       `useAsyncData`（内容页必须进服务端 HTML）；**前端一次都不排序**，因为后端契约
>       就是"月份倒序、月内也倒序"，前端再排一次等于把同一条规则维护成两份；
>       接口挂了只降级成一句"归档暂时读不到" + 回首页入口，**不做成整站错误页**；
>       "还没有文章"与"读不到"是两句不同的话（一个让人去写第一篇，一个让人稍后再来）
>     · RSS `/feed.xml`：Nitro 运行时路由（与 sitemap 同一套路，**不是构建时生成** ——
>       构建期连不上后端，发新文章也不该重新构建镜像）；`<pubDate>` 逐字段拼成
>       RFC 822，**不把无时区的时间串交给 `new Date()`**（同一份数据在服务器和用户浏览器上
>       会解析成两个时刻）；不存在的日期（2 月 30 日）直接不写这条，**绝不用当前时间兜底**；
>       `<description>` 放后端摘要而不是 Markdown 源码；接口失败时返回**合法但只有站点信息**
>       的 feed，**绝不 500、绝不空 body**（订阅器把这两种当成"源坏了"并停止抓取）
>     · 我复核过的部分：读了 `app/pages/archive.vue`、`server/utils/feed.ts`（218 行）、
>       `server/routes/feed.xml.get.ts` 全文与两个提交的 diff —— 契约、降级路径、
>       转义复用（与 sitemap 共用 `escapeXml`）、`toCount` 确实是 `app/composables/useSiteStats.ts:51`
>       的真实导出（自动导入）都对得上；`/archive` 也补进了 sitemap 的 `STATIC_PAGES`
>     · 两处**只有靠测试才拦得住**的东西：归档页 `dayOf()` 的正则一开始抓的是**小时**
>       （页面上所有文章都印同一个小数字，肉眼看着完全像正常数据），是"每篇只印自己那天"
>       这条断言把它拦下来的；feed 的"XML 合法"断言自带一个良构扫描器**并配了对照组**
>       （喂未转义的 `&`、裸 `<`、标签不配对，断言它真的会报错），否则一个永远返回 null
>       的检查器会让"XML 合法"永远为真（假绿）
>
> **⚠️ 一处需要前端补的分类管理**：后端 `POST/PUT/DELETE /admin/category` 已经就绪，
>   前端 w6.1c 当时只做了只读列表。这一批已经把它补成可编辑（见 w6.1d 行）。

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w6.1** | ✅ `新增文章标签的展示与按标签筛选，后台文章可多选标签` | 标签云 / 详情标签 / `?tagId=` 筛选（后端 6.1 + 6.1b） | 用例：标签跳转带上筛选参数；进入页面时标签也走 SSR —— **已完成** |
| **w6.1d** | ✅ `新增后台分类管理，把只读列表换成可编辑`（`ecc276d`） | 把 w6.1c 那版**只读**分类列表换成完整增删改（后端 `POST/PUT/DELETE /admin/category` 当时就已就绪，所以**没有再动后端**） | 用例 19 条：请求形状与后端 `CategoryForm` 对齐、trim、重名与非空校验在弹窗里留住、**删除被拒时后端那句「还有 N 篇文章在用」原样留在页面上**（不是三秒就没的 toast）、404 自愈、描述为 null 显示「—」—— **已完成** |
| **w6.2** | ✅ `新增文章评论组件` | 发表 / 列表 / **待审核提示**；被限流时走 429 文案（后端 6.2） | 用例：提交成功后提示"等待审核"、校验拦截、内容按纯文本渲染 —— **已完成** |
| **w6.2b** | ✅ `新增后台评论审核页面` | 待审核列表 + 通过 / 拒绝 / 删除（后端 6.2） | 用例：默认筛选待审核、审核调用正确接口、删除、列表刷新 —— **已完成** |
| **w6.3** | ✅ `新增归档页，按年月浏览全部文章` + `新增 RSS 订阅，并提供 feed.xml`（**拆成两个提交**：归档与订阅是两件独立的事，合在一起会让"订阅出问题要回滚"顺带把归档页也带走） | 归档按年月分组（`ec5fee8`）、`/feed.xml`（`6b5b9ec`）；两者都走**后端已有的**数据接口，没有为此改后端 | 用例：归档分组正确（14 条，含**故意喂乱序输入**断言前端一次都不排序）、feed 是合法 XML 且含最近文章（28 条，自带良构扫描器 + 喂坏数据做对照组）—— **已完成，见上一条进度块** |

### web-7 · SEO 与性能（前端专属，1 天）—— ✅ 已完成（w7.1 / w7.2 / w7.3 / w7.4）

> **w7.4 已完成并复核**（提交 `f4d4fbc`，2026-09-11）—— `admin.vue` **2141 → 681 行（−68%）**，
> 按菜单拆成 `app/components/admin/` 下 7 个组件 + 一个共用工具：
> `OverviewPanel` 82 · `ArticlesPanel` 194 · `ArticleEditDialog` 386→**394** ·
> `UsersPanel` 310 · `TagsPanel` 219 · `CategoriesPanel` 302 · `CommentsPanel` 293 行，
> 再加 `app/utils/time.ts` 19 行（四个面板都要用的 `formatDateTime()`）。
> **我逐个核过行数：9 个里 8 个与提交信息里的数字完全一致**；
> 唯一不一致的是 `ArticleEditDialog.vue` —— 提交信息与 README 都写 386，实测 **394**
> （它在那之后又加了 8 行）。已让它改，这类"文档数字过期"正是同一个提交里它自己刚修掉
> `2138 → 2141` 的同一类问题
>
> 这一条里**最值得记的是三条判断**（都不是"把大文件切开"这么简单）：
>   · **划分依据不是"看哪个文件大"，而是"一个菜单 = 一个独立的加载/刷新单元 +
>     一个能独立改坏的单元"**。切到哪个菜单要重拉什么，恰好就是那个菜单的整块内容，
>     于是"什么时候该刷新"和"哪一段该重读"变成同一件事。文章弹窗再单独拆，
>     是因为它是这一页里唯一的重依赖（Markdown 编辑器）且自己扛着整张表单状态
>   · **跨菜单共享的状态一份都没有复制**（复制必然出现"两个地方看到的不是同一批数据"，而界面上看不出来）：
>     `articles/artTotal/artQuery` 同时喂文章表格与概览的"最近文章"、`categories` 同时喂筛选下拉 +
>     文章弹窗 + 分类管理页、`tags` 喂标签多选 + 标签管理页、`pendingCommentCount` 喂菜单角标 + 评论页标题。
>     传法：只读的走 props，父子共用的对象/数字走 `v-model`；向上只抛"要一份新的"（`refresh` / `refresh-tags` / …），
>     **谁持有数据谁负责读接口**
>   · **样式是比逻辑更容易出事的地方**：Vue 的 scoped 样式只把 scope 属性打在子组件**根元素**上，
>     而四个弹窗还会被 `el-dialog` teleport 到 `body`（物理上已不在 `.admin` 里）。所以面板独有的样式跟着面板走；
>     多面板共用的展示规则放进全局块并**统一加 `.admin ` 前缀**（不加会漏到首页与归档页 ——
>     它们自己有同名 `.glass` 的 scoped 定义，两边会互相打架）；弹层与 Element Plus 的覆盖
>     则**不加前缀**（它们在 body 上，加了反而谁也匹配不到）。实测后台那份 CSS 只差 15 字节
>   · **它如实交代了唯一一处可观察差异**：打开"编辑文章"原来是
>     `await Promise.all([fetchTags(), request(详情)])`，现在标签那次改成抛事件请父页面去拉
>     （事件监听器拿不到 promise），所以只 await 详情。两者**同时**发出，
>     差别仅在"标签接口比详情慢"时：弹窗早几十毫秒出现、下拉选项稍后补齐 ——
>     请求形状/结果/界面内容不变，属时序差异。**这种"重构带来的细微行为变化"最容易不写，它写了**

> **w7.3 已完成并复核**（提交 `fa251b9`，2026-09-11）—— 这一条里有**三件只有真跑一遍才会发现**的事：
>   · **①计划里写的手工方案会静默丢掉 `ElMessage` / `ElMessageBox` 的样式**：
>     `unplugin-vue-components` + `ElementPlusResolver` 只处理"模板里用到且**没有**被 import"的组件，
>     而本项目有四个文件是**显式** `import { ElMessage } from 'element-plus'` 的
>     （`app.vue` / `useApi.ts` / `index.vue` / `admin.vue`），`admin.vue` 还用着 `ElMessageBox`
>     —— 实测构建产物里 `.el-message` 出现 **0 次**，表现是"所有错误提示与二次确认框都是没样式的裸文本"，
>     **不报错、不让任何用例变红**（组件测试断言的是文字与 DOM，从不读 CSS）。
>     所以最终改用官方模块 **`@element-plus/nuxt`**（它对样式的判断多一条路：扫源码里那四个函数式 API 的标识符）
>   · **②显式 import 一个"组件"会让它的样式不被注入**（`ElMessage` 这类函数式 API 不受影响）：
>     模板里直接写 `<el-button>` → 编译成 `_resolveComponent("el-button")`，模块抓得到；
>     `import { ElButton }` 之后再写 → 编译成 `_createBlock($setup["ElButton"], …)`，抓不到
>   · **③官方模块的代价**：它在**配置加载期**就 `import * as AllComponents from 'element-plus'`，
>     而 `nuxt.config` 由 jiti 逐文件执行 —— 单测里"把 Nuxt 环境搭起来"从 **2.9 秒涨到 11 秒**，
>     超过 `@nuxt/test-utils` 默认 10 秒的 hook 超时，后果是 **25 个测试文件里 10 个连一条用例都没跑到**
>     （输出像"238 passed / 228 skipped"，很容易被当成"只是慢"）。修法是显式设 `hookTimeout: 60_000`
>     —— 加的是环境搭建的时间，**没有放宽任何断言**
>   · **我（复核方）独立验过的部分**（不是照抄提交信息）：产物 JS **2 483 252 字节**、
>     CSS **299 419 字节**，与提交信息里的改后数字**逐字节一致**；CSS 里用到的组件样式都在
>     （`.el-message` **99** 处 —— 正是手工那版会丢的那个），没用到的 12 个组件样式（carousel /
>     calendar / cascader / transfer / color-picker / tree / timeline / steps / splitter /
>     watermark / rate / slider）**全部 0 处**；客户端 JS 里也搜不到没用到的组件代码，
>     旧的 `element-plus/dist/index.css` 引用 **0 次**
>   · ⇒ **`icon: false` 这条它的口径我没能复现**（README 里写"`.nuxt/components.d.ts` 从 104 423 降到 6 157 字节"，
>     而磁盘上那份是改前的 104 423 字节、mtime 早于最后一次构建）：`.nuxt` 每次 dev/build/test 都会重建
>     且本身被 gitignore，按字节数写注定过期。已让它改成效果式检查（或重测并写明时点），
>     可复现的替代证据是"客户端产物里 `icons-vue` 出现 **0** 次"
>
> **w7.1 / w7.2 已完成并复核**（提交 `389544d` / `28640b7`）：
>   · 首页、详情页、后台页都接上了 SEO；`app/utils/seo.ts`（纯函数）+ `useSeoMetaFor`
>   · **没有封面时整条 `og:image` 都不出现**（空值等于声明一张空图，
>     分享出去是空白卡片，而且只有真去分享才看得出来）
>   · 首页 canonical 固定为不带参数的 `/`（搜索/分类结果页与首页是同一份内容，
>     避免重复内容）；后台页 `noindex, nofollow`
>   · **sitemap / robots 用 Nitro 运行时路由，不是构建时生成**：前端镜像的
>     docker build 连不上后端，构建期只能得到"构建失败"或"一份只有首页的 sitemap"；
>     而且发新文章不该重新构建镜像。取文章时**分页循环**（后端单页上限 50），
>     三种终止条件（空页 / 已取满 total / **40 页上限防死循环**），
>     失败降级成"只含首页的合法 XML"，绝不 500、绝不空 body
>   · 域名与 canonical **同源**（都由 `seo.ts` 的 `absoluteUrl` 拼），
>     避免"canonical 指向 A 域名、sitemap 指向 B 域名"这种只有对比才发现的问题
>   · 我复核过的部分：`eslint` 0 问题、**280 用例 / 16 文件全绿**、`npm run build` 通过、
>     产物里确实有 `chunks/routes/sitemap.xml.get.mjs` 与 `robots.txt.get.mjs`
>   · 代理实测（写进了提交信息的【联调】节）：生产服务 curl 首页 HTML 里有
>     `<title>` / `description` / `canonical` / 四条 og / `lang="zh-CN"` **与真实文章卡片**；
>     后端挂掉时 meta 一个不少、`/sitemap.xml` 仍返回 200 + `application/xml`（降级版）
>   · ⚠️ 部署时要注意：`/sitemap.xml` 与 `/robots.txt` 必须由 Nginx 转发给前端容器
>     （它们是运行时路由），**不能配成静态文件**；后端 README 的 Nginx 章节已写明
>
> **另外顺手做掉的两件（不在原计划里，但都是真问题）**
>   · `$fetch` 默认会对 **429 与 5xx 自动重试一次** —— 限流时前端会把请求翻倍打过去，
>     正好和限流的目的相反。改成"429 不重试、5xx 仍重试一次"（用假 fetch 驱动真 ofetch
>     数调用次数验证：429 → 1 次、对照组 → 2 次）
...
| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **w7.1** | ✅ `首页改为服务端渲染，并补齐全站SEO元信息` | `index` 从 `onMounted` 改 `useAsyncData`；title/description/OG/canonical（对应 C1 / C2） | 构建后首页 HTML 里**含文章列表与 meta 标签** —— **已完成** |
| **w7.2** | ✅ `新增sitemap.xml与robots.txt，都由Nitro运行时路由提供` | 运行时路由（不是构建时生成，理由见上）+ 分页取全量 + 失败降级 | 产物里有 sitemap/robots 路由；curl 返回合法 XML —— **已完成** |
| **w7.3** | ✅ `把 Element Plus 改为按需引入，产物 JS 与 CSS 各减 17.9% 和 42.5%`（`fa251b9`） | **没有按计划用手工 unplugin**，改用官方 `@element-plus/nuxt`（对应 D4）—— 为什么换见上方进度块第 ①②条；`icon: false` 关掉 293 个图标的全局注册 | 构建通过且产物**真的变小**（JS −17.9% / CSS −42.5%，我逐字节复核过）；另加 4 条护栏用例，从 DOM、体积、样式三个方向守"坏掉时不报错"这件事 —— **已完成** |
| **w7.4** | ✅ `拆分后台单文件组件，admin.vue 从 2141 行降到 681 行`（`f4d4fbc`） | 按菜单拆成 7 个组件 + `app/utils/time.ts`（对应 D3）；共享状态全部留在页面、一份都没复制；样式按 scoped / 全局加前缀 / 弹层不加前缀三类分别处理 | 拆分前后**用例数不变、断言一条没放宽**（三处"够不着"的断言改成先 `findComponent` 再读组件内部，比对的还是同样那几件事）；`npm run test` 26 文件 / 470 用例全绿 —— **已完成**（26 / 470 与产物体积 JS 2 487 246、CSS 299 404 我已独立实测复核） |
...
| **3.2** 封面上传 | **w3.1** 上传组件 | 后台上传封面 → 前台卡片和详情显示图片（**存服务器本地磁盘**，不是 OSS）；传 `.exe` / 超大文件被拒并有提示 —— **已完成** |
| **3.3** Dockerfile + 三容器 | **w3.3** 前端 Dockerfile | `docker compose up -d` 一次性起全栈 → 浏览器访问域名可用 —— **本机已按生产形态完整演练过（四容器 + 空库 6 个迁移 + SSR 首页/文章页 + sitemap/robots + 评论链路 + 审计 + 上传落卷 + 限流），见后端 README「部署」** |
| **3.4** 部署文档 | **w3.4** 前端 README | 手机 4G 打开域名：能发文、能看到图、能登录后台 |
| **3.5** 管理员初始化引导（新增） | —— | 用一个**全新空库**启动后端 → 能创建出管理员并登录后台（当前会卡在没有管理员这一步） |
| **4.1** 限流 | **w4.1** 429 处理 | 连点登录 6 次 → 前端提示"请求过于频繁"（**不是**"网络异常"）—— **已完成**；本机实测后端 `200,200,200,200,200,429`，429 的 body 是 `{"code":429,...}` |
| **4.2** 操作日志 + `@Async` | —— | 后台写操作后前端无感；`operation_log` 有记录（**已完成**：当时 8 类写操作，异步落库，请求耗时不增加；**现在一共 29 类** —— 标签 / 分类 / 评论之后加了友链 / 项目 / 收藏 / 关于 / 音乐，见 §21.4、§21.5） |
| **5.1** logback + traceId | **w5.3** 错误带 traceId | 前端报错时拿 traceId 去后端日志里能定位到同一次请求 —— **已完成**（头名是 `X-Trace-Id`；本机实测正常响应与 429 响应都带它，且 CORS `Access-Control-Expose-Headers` 已暴露给浏览器） |
| **5.2** 登出 token 失效 | **w5.1** 登出链路 | 点"退出" → 后端收到登出 → 再点需要登录的操作 → 401 → 弹登录框且 `user` 已清空 —— **已完成** |
| **5.3** 索引优化 | —— | 首页列表加载主观/客观变快 |
| **5.4** 幂等 | **w5.4** 防连点 | 连点"发布"两次 → 只产生一篇文章，提示不重复 —— **已完成** |
| **5.6** 错误码统一（破坏性） | 前端错误分支同步改 | **两端必须在同一时间窗内完成**；改完后所有错误提示走 HTTP 状态码 |
| **5.7** Prometheus 指标 | —— | 不涉及前端 |
| **5.8** 连接池 + 压测 | —— | 不涉及前端 |
| **5.9** 安全响应头 + 凭据治理 | —— | 不涉及前端（响应头由浏览器/网关消费） |
| **6.1a / 6.1b** 标签与打标签 | **w6.1 / w6.1b**（`1e48289`）标签展示 + `?tagId=` 筛选 + 后台文章多选标签 | 后台建标签 → 文章打标签 → 前台按标签筛文章 —— **已完成**（tagId 并进原有的 `useArticleFilter`，不另起一套同步逻辑） |
| **6.1c** 分类增删改 | **w6.1c**（`e559910`）「分类 / 标签」占位菜单换成两个真实页面（分类当批只做只读） | 后台改名 → 前台立刻生效 —— **已完成** |
| **6.2** 评论与审核 | **w6.2 / w6.2b**（`bc1d1d6` / `8079bdf`）评论组件 + 后台审核页 | 前台发评论（游客可发、默认待审核）→ 后台通过 → 前台可见；待审与已拒的不可见 —— **已完成** |
| **6.4** 归档接口 | **w6.3**（`ec5fee8`）归档页 | 打开 `/archive` → 按年月分组、**顺序就是后端给的顺序**，且在服务端 HTML 里（爬虫不执行 JS 也看得到全部内链）—— **已完成** |
| **6.5** RSS 数据接口 | **w6.3**（`6b5b9ec`）`/feed.xml` | 订阅器抓 `/feed.xml` → 合法 XML + 最近 20 篇；后端挂掉时仍返回**合法**feed（只有站点信息）、**绝不 500 也绝不空 body** —— **已完成** |
| **6.1c 补**（后端写接口当时就已就绪） | **w6.1d** 后台分类可编辑 | 后台新增 / 改名 / 删除分类 → 前台筛选里立刻反映；**有文章在用时的删除要被后端拒绝，前端如实提示** |
| **6.3** RabbitMQ | —— | 评论后通知异步到达，接口响应不阻塞（**可选，未做**） |
| —— | **w7.1 ~ w7.4** SEO / 性能（前端专属） | 构建后首页 HTML 含文章列表与 meta；sitemap 可访问 |
| **F6** 音乐模块（`GET /music/list` + `/admin/music/**` + `POST /upload?type=audio`） | **F6 前端**：音乐页改读接口 + 后台曲目管理面板 | 后台上传 mp3 + 填曲名/歌手/封面/歌词 → 前台音乐页出现这首、能播、歌词按 LRC 显示；删掉之后前台回落成内置那一首（`/media/bg-music.mp3`）—— **后端已完成（见 §21.5），前端待做** |
...
| 3 | 手写 `SETNX` 互斥锁防缓存击穿 | **`@Cacheable(sync = true)`**（本项目用的是这个：单实例部署，框架内部对同一 key 加互斥）—— 跨实例时才需要 Redisson `RLock`（本项目**没有**引入） | `SETNX` 的锁续期、误删、可重入全要自己写，而 `sync = true` 一行注解就够；两者都会**阻塞**等待重建，跨实例/要"返回旧值不阻塞"时才轮到逻辑过期 | 2.2 |
...
| 6 | 自研 `@Idempotent` 注解 + AOP + `SETNX` | **Redis `SET NX` 占位 + 结果缓存**（`IdempotencyService`，用 `setIfAbsent(key, value, Duration)`）；**没有用数据库唯一索引，也没有引入 Redisson** | 要把"防重复提交"做对，一个带 TTL 的幂等键就够；`SET NX` 一条命令天然原子，不存在锁续期/误删/可重入那些要自己写的东西；也不需要往每张业务表加唯一列 | 5.4 |
...
| ✅ 换成 | `Resilience4j（限流与熔断降级）`、`Nginx 限流`、**`@Cacheable(sync = true)`（防击穿）**、**Redis `SET NX`（接口幂等）**、`Micrometer Tracing 链路追踪`、`Spring 事件驱动` —— ⚠️ **没有 `Redisson`**：它在选型时被评估过（跨实例方案），但单实例部署下 `@Cacheable(sync=true)` 与 `SET NX` 就够了，最终**没有引入这个依赖** |
...
## 21. F 批：前端补全与修复（2026-09-11，用户用完一轮之后的反馈）

> 这一批**不在原来的 A–E 批里**，是用户实际点过一遍之后提出的缺口与问题。
> 规则与前几批完全一致（§15 R1–R8）：**一个功能一个提交 → 先写测试 → 绿测 → 同步 README → 提交**，
> 前后端各自提交后做一次联调（§19）。下面每条都先写清"**用户看到了什么**"与"**我核实的根因**"，
> 因为这一批里有三条是**代码写错了**、不是"还没做"。

### 21.1 用户报的问题 → 我核实的根因

| # | 用户描述 | 核实的根因 | 性质 |
|---|---|---|---|
| **F0-a** | 归档页的数据与文字"没有表格框住、与背景融为一体" | `archive.vue` 给月份卡片与三个状态块都写了 `class="glass"`（第 17/18/22/30 行），**但该文件里没有任何 `.glass` 规则**；全项目只有两处定义：`index.vue` 里是 **scoped**（只对首页生效）、`admin.vue` 里是**全局但加了 `.admin` 前缀**（只在后台内生效）。⇒ 归档页那些元素**没有背景/边框/毛玻璃**，文字直接压在背景视频上 | 🔴 **真 bug**（缺 CSS，不是审美问题） |
| **F0-b** | 评论管理列表里"按钮堆"在一起 | `CommentsPanel.vue` 的 `操作` 列宽只有 **176px**，里面塞了「通过 / 拒绝 / 删除」三个按钮 → 换行堆叠 | 🟡 布局 |
| **F0-c** | 超长评论看不到全貌，要"查看详情" | 内容列只有 `show-overflow-tooltip`（要悬停才看得到），没有详情入口 | 🟡 缺功能 |
| **F1** | 右上角 ⚙ 点下去没反应 | `app.vue:38` 是 `@click="onDev"` | 🟡 占位 |
| **F2** | 「文章」下拉的技术 / 读书 / 随笔都显示"功能开发中" | `app.vue:28` 三项都调 `onDev()` —— 而**后端已经有真实分类**（技术笔记 / 项目复盘 / 生活随笔），可以直接复用 `?categoryId=` 筛选 | 🟡 占位（有现成接口可接） |
| **F3** | 音乐点进去没有东西 | 导航「音乐」只是 `/#music` 锚点到首页卡片；**没有独立页面**，卡片本身也只有静态封面 + 一个播放键 | 🟡 缺页面 |
| **F4** | 收藏 / 项目 / 友链 / 关于"待开发" | 前三项 `onDev()`；「关于」是 `/#profile` 锚点 | 🟡 缺模块（**用户已选：后端建表 + 后台可管理**） |
| **F6** | 窄屏（手机 / 小窗）下**导航标签全部丢失** | `app.vue` 里那行 `@media (max-width: 1000px) { .nav-center { display: none; } }` —— **不是被挤窄，是整个中间导航被直接隐藏**（首页 / 归档 / 文章 / 音乐 / 收藏 / 项目 / 友链 / 关于 全没了），而且**没有任何替代入口**，窄屏只剩 Logo 与登录按钮 | 🔴 **真 bug**（移动端等于没有导航） |

### 21.2 素材现状（已实测，决定音乐页的实现方式）

- `static-media/bg-music.mp3` 是 `ID3v2.4`，**标签体只有 138 字节且一个帧都没有**
  （`TIT2` / `TPE1` / `TALB` / `APIC` / `USLT` 全部不存在）
  ⇒ **文件里既没有自带封面、也没有歌词**。所以做法定为：
  · **封面**：`jsmediatags` 不在依赖里，自己按 ID3v2 规范读 `APIC` 是可控的（只读文件头若干字节）；
    **优先读内嵌封面，读不到就用 `public/cover-1.png` 兜底** —— 用户把封面写进 mp3 之后
    **不需要改代码**就会自动生效
  · **歌词**：读 `static-media/bg-music.lrc`（线上经 Nginx 的 `/media/`），
    **没有就显示「暂无歌词」并提示放置路径** —— **不编造歌词**（编一份假歌词比空着更糟）
- 结论：能力按标准做法实现，素材缺口用**明确的兜底与提示**处理，而不是假装有。

### 21.3 分阶段提交清单（F0–F4）

| # | 提交标题 | 内容 | 测试要求 |
|---|---|---|---|
| **F0-a** | `修复归档页缺少玻璃底样式，内容与背景糊在一起` | ① 归档页补上自己的 `.glass` 规则（**不再依赖别的页面的 scoped 样式**）② 给月份块与文章行加可见的表头/边框/行分隔，让它像"表格"而不是飘着的字 ③ 顺手修掉 `admin.vue` 里那句"归档页自己有 .glass scoped 定义"的**错误注释** | 用例：归档页月份块与文章行带预期 class；**并加一条"归档页的样式不再依赖别的页面"的护栏**（例如断言该文件里存在 `.glass` 定义）；分组与排序断言保持不变 |
| **F0-b** | `修复后台评论列表操作列按钮堆叠` | 操作列加宽或把「拒绝/删除」收进下拉；窄屏下不换行 | 用例：三个操作按钮仍然都能点到、且发出的请求与之前一致（断言请求形状，不改行为） |
| **F0-c** | `新增后台评论的查看详情` | 长评论给一个「详情」入口，弹窗展示完整内容（含邮箱/IP/时间/文章） | 用例：超长内容在列表里被截断、详情弹窗里是**完整**内容；短评论也有一条（不该出现"详情"或行为一致） |
| **F1** | `新增设置面板，可开关背景视频与背景音乐` | ⚙ 打开面板，两个开关：背景视频、背景音乐；选择存 localStorage；与页脚现有的音乐开关**共用一份状态**（不能出现两处开关互相打架） | 用例：开关切换 → 状态落 localStorage；重进页面按存储恢复；两个入口（⚙ 与页脚）互相同步 |
| **F2** | `把导航「文章」下拉改为按真实分类跳转` | 下拉项改为读取 `/category/list`（SSR 可取），点击跳 `/?categoryId=N`；不再出现"功能开发中" | 用例：分类接口返回什么就渲染什么；点击后的 URL 带正确 `categoryId`；接口失败时下拉降级成"全部文章"而不是空菜单 |
| **F3** | `新增独立音乐页，首页音乐卡改为它的迷你版` | `/music`：旋转唱片（内嵌封面 → `cover-1.png` 兜底）、当前歌词行、歌词列表（可点击跳转）、播放/暂停/进度/音量；`app/utils/lrc.ts`（LRC 解析纯函数）与 `app/utils/id3.ts`（读 APIC 封面）都写成可单测的纯函数；首页 `#music` 卡片复用同一套组件的小号版本 | 用例：LRC 解析（时间戳格式、乱序、`[00:00.00]` 空行、没有 LRC 时显示"暂无歌词"）；ID3 解析（有 APIC / 无 APIC / 文件头不是 ID3 / 长度不足）；当前行随播放时间切换；首页迷你卡与完整页**读同一份数据** |
| **F6** | `把窄屏导航改成汉堡菜单，不再整块隐藏` | 根因是那行 `@media (max-width: 1000px) { .nav-center { display: none; } }`：**导航在窄屏被整块藏掉、且无替代入口**（手机上只剩 Logo 与登录按钮）。做法：≤1000px 显示汉堡按钮，点开一个面板/抽屉，里面是**同一批导航项**（含「文章」下来的真实分类）；保留登录/退出在顶栏；要点：`aria-expanded` / `aria-controls` / `aria-label`、Esc 关闭、点遮罩关闭、**路由跳转后自动关闭**、打开时锁 body 滚动；**显隐一律用 CSS 媒体查询控制，不用 JS 读窗口宽度**（否则 SSR 首屏会闪一下、还可能 hydration 不一致） | 用例：汉堡按钮存在且可切换、`aria-expanded` 跟着变；点菜单项后关闭；Esc 与点遮罩都能关；**窄屏/宽屏的显隐是 CSS 负责**（所以断言"两套结构都在 DOM 里、由状态与媒体查询控制"），并在 README 的上线核对清单里加一条人工检查（手机宽度下导航可用） |

**执行顺序（有意安排的）**：`F0-a` / `F0-b` / `F0-c`（三个已确认的 bug，先修掉）
→ **`F6` 汉堡菜单**（放在 F2/F4 之前：F2 会把真实分类塞进下拉、F4/F5 还会再加四个页面入口，
导航只会更长；先把"窄屏整块隐藏"换成汉堡，后面加项就不用返工两遍）
→ `F1` 设置面板 → `F2` 文章下拉接真实分类 → `F3` 音乐页 → `F5` 四个内容模块（后端 + 前端各自提交）。

### 21.4 F5：内容模块（收藏 / 项目 / 友链 / 关于）—— 用户已选"后端建表 + 后台可管理"

> ### ✅ 后端部分已完成（2026-09-11，4 个提交）；**前端部分待做**
>
> | 提交 | 内容 |
> |---|---|
> | `0075e12` | 新增友链模块：建表、公开只读列表与后台增删改，并接进缓存与操作审计 |
> | `ce44db3` | 新增项目模块：建表、公开只读列表与后台增删改，并补一条跨字段的业务规则 |
> | `e1f5796` | 新增收藏模块：建表、公开只读列表与后台增删改，并写清分组字段的语义 |
> | `49dde9d` | 新增关于模块：单条记录的表、公开读取与后台保存，并钉住三条单条记录的不变量 |
>
> **验证**：全量 `mvnw -B test` → **369 用例 / 0 失败**（基线 285，+84：FriendLinkTest 20 /
> ProjectTest 23 / FavoriteTest 21 / AboutTest 16 / OperationLogTest +4），行覆盖 **91.2%**；
> 联调在自起实例（8083，**没动用户 8082 那个**）逐条实测成功路径与 400/404/405/401/403，
> 联调后**数据全部复原**（四张新表 0 行、`about` 那行改回初始值、探针账号物理删除、
> Redis 相关 key 清掉）。dev 库已被 Flyway 应用 V7–V10（四张新表已存在且为空）。
>
> **⚠️ 前端要按这些契约来接（后端有意为之，不是疏漏）**
> - 字段名：`about` 的其它联系方式是两个字段 **`wechat`（≤50）/ `qq`（≤30）**，都只卡长度不做格式校验；
>   `bio` 上限 5000 字、存 **Markdown 原文**（后端不转 HTML —— 前端若用 `v-html` 渲染必须自己净化）
> - `project.tech` 是**逗号分隔字符串**（前端自己 `split(',')`）；
>   `favorite.category` 是**自由文本分组名**，**不是** `category` 表（后者是文章栏目）
> - 地址字段：`link.url` / `project.url|repo` / `favorite.url` 必须是 `http(s)://` 绝对地址；
>   `avatar` / `cover` 额外允许 `/` 开头的站内路径（因为封面是上传到本机的）
> - `project` 的 `url` 与 `repo` **至少要填一个**（跨字段规则，写库前校验）
> - **四个公开接口都不分页**；`GET /about` 返回**对象**而不是数组
> - 后台保存一律返回 `Result<Void>`（要回显就再 GET 一次）
> - `about` 是**单条**：只有 `PUT /admin/about`，`POST` 是**真 405**（有用例守着）；
>   那一行被物理删掉时读接口会返回空壳、保存会自愈写回（**有意放宽**，代码与提交信息里都写明了）
> - 四个公开列表共用 Redis 版本号 **`content:list:version`**（key 形如 `link:list:v1:{版本号}`，
>   TTL 5 分钟 + 抖动）—— **刻意不复用 `ArticleCacheVersion`**，否则改一条友链会把
>   文章列表/详情/归档/RSS/分类/标签六份缓存一起作废
> - 连带影响已全部落地：`OperationAction` **16 → 26**、`AuditTarget` **5 → 9**、
>   接口 **36 → 53**、权限矩阵算式 **`16 + 1 + 36 = 53`**、测试清单 **35 类 369 用例**
>   （⚠️ **这四个数字是 F5 当时的快照**；加了音乐模块（§21.5）之后分别是
>   `26 → 29`、`9 → 10`、`53 → 58`、`36 类 403 用例` —— 看新数字请翻到 §21.5）

> 这一块是这批里最大的一块，**按模块拆成"后端一个提交 + 前端一个提交"**，做完一个模块联调一次。
> 四个模块的表结构、接口、缓存与审计**全部沿用现有惯例**（与 `tag` / `category` / `comment` 对齐）。

**表（一个模块一个迁移，延续 V5 建 tag 表 / V6 建 comment 表的做法）**

| 迁移 | 表 | 字段要点 |
|---|---|---|
| `V7` | `friend_link` | name / url / avatar / description / sort / status / create_time / update_time / deleted |
| `V8` | `project` | name / description / url（在线地址）/ repo（仓库）/ cover / tech / sort / status / 时间与逻辑删除 |
| `V9` | `favorite` | title / url / description / category / sort / status / 时间与逻辑删除 |
| `V10` | `about` | **单条记录**：nickname / avatar / bio（长文本）/ email / github / 其它联系方式 / update_time |

**接口（公开只读一律 `GET` + 走缓存；后台一律类级 `@PreAuthorize("hasRole('ADMIN')")`）**

| 模块 | 公开 | 后台 |
|---|---|---|
| 友链 | `GET /link/list` | `GET /admin/link/list`、`POST /admin/link`、`PUT /admin/link/{id}`、`DELETE /admin/link/{id}` |
| 项目 | `GET /project/list` | `GET/POST/PUT/DELETE /admin/project…` |
| 收藏 | `GET /favorite/list` | `GET/POST/PUT/DELETE /admin/favorite…` |
| 关于 | `GET /about` | `PUT /admin/about`（单条，只改不建） |

**连带影响（必须同一次提交里一起改，否则又是"文档与代码不一致"）**

- `OperationAction` 会从 **16 个常量涨到 26 个**（友链 / 项目 / 收藏各 3 个增删改 = 9，关于只有 1 个 update）
  ⇒ README 里"共 16 类管理动作"、`OperationLogTest` 的描述、路线图 §9/§21 的数字都要跟着改
- `AuditTarget` 会从 **5 个取值涨到 9 个**（新增 `LINK` / `PROJECT` / `FAVORITE` / `ABOUT`）
  ⇒ 枚举注释与 README 的取值清单一起改
- 接口总数从 **36** 涨到 **36 + 4 公开 + 13 后台 = 53**（关于是"1 公开 + 1 后台"）
  ⇒ README 的接口表、权限矩阵（矩阵开头的 `12 + 1 + 23 = 36` 那条算式）、路线图 §19 都要同步
- 前端：四个页面 + 后台四个管理面板 ⇒ 后台菜单从七个变十一个，`admin.vue` 与 README 的后台结构说明一起改

**每个模块的测试（沿用"每功能至少四类"的标准）**：正常路径（并且校验数据库真的写进去了）、
边界（空值 / 超长 / 非法 URL / 排序为 null）、权限（无 token → 401、游客 → 403）、
失败路径（不存在 → 404、非法参数 → 400），外加"公开只读的缓存命中与写操作失效"。

---

### 21.5 F6：音乐模块（后台录曲目 + 音频上传）—— **后端已完成，前端待做**

> **用户原话**：「后台添加音乐上传」—— 曲目不再由前端写死，而是后台上传 mp3 + 填曲名 /
> 歌手 / 封面 / 歌词，存进库，前台播放器读接口渲染。
> 这一节是**完成后补记的**（含真实数字与实测证据），规则与前几批一致（§15 R1–R8）。

| 提交 | 内容 |
|---|---|
| **F6** | 新增音乐模块（`V11__create_music_table.sql` + 公开只读列表 + 后台增删改 + Redis 缓存 + 操作审计），并给 `POST /upload` 加 `type=audio`（mp3 / 20MB，存 `uploads/music/`） |

**表**：`V11` 建 `music`（title / artist / url / cover / lyrics(TEXT) / sort / status / 时间与逻辑删除），
索引 `(status, sort, id)` —— 比 V7–V9 多带一个 `id`，因为前台那条查询是 `ORDER BY sort, id` 两段排序。
⚠️ **迁移里刻意不插种子数据**：前端在列表为空时会回落成内置的那一首（`static-media/bg-music.mp3`），
所以"库里一首都没有"是正常状态，不是漏了数据（这一点在迁移与 Service 注释里都写明了）。

**接口**（公开 1 + 后台 4，与 F5 四个模块同形）

| 模块 | 公开 | 后台 |
|---|---|---|
| 音乐 | `GET /music/list`（只含 `status = 1`，`sort ASC, id ASC`，走缓存；空列表返回空数组） | `GET /admin/music/list`、`POST /admin/music`、`PUT /admin/music/{id}`、`DELETE /admin/music/{id}` |

**上传的两套规则（这次动了已有的上传接口，所以单独说清）**

| | 图片（默认 / `type=image`） | 音频（`type=audio`） |
|---|---|---|
| 扩展名 | `jpg` `jpeg` `png` `gif` `webp`（**没变**） | `mp3` |
| 大小上限 | 5MB（**没变**） | 20MB（依据：320kbps × 5 分钟 ÷ 8 ≈ 12MB，留余量取 20MB） |
| 存储目录 | `uploads/cover/yyyy/MM/` | `uploads/music/yyyy/MM/`（静态映射 `/uploads/**` 已覆盖子目录） |
| 返回结构 | `{"url": "..."}` | **完全相同**（前端上传组件不用改） |

- **不传 `type` 与 `type=image` 是同一个结果**（保持接口上线以来的行为），
  未知的 `type`（比如打错的 `audo`）直接报错而**不**回落到图片 ——
  回落会让参数写错表现为"格式不支持"，提示指错地方
- **两个方向都不许互相放宽**，各有用例钉住：⑭ `type=audio` 传 `.png` → 拒；
  ⑮ 不传 `type` 传 `.mp3` → 拒；⑰ 同一份 1.5KB 内容作为图片被拒（图片限 1KB）、作为音频被接受（音频限 2MB）
- ⚠️ framework 层的 `spring.servlet.multipart.max-file-size` 从 5MB 提到 **20MB**
  （必须 ≥ 音频上限，否则大文件根本走不到业务代码）；
  **图片的 5MB 由业务层按 `type` 拦下**，两条防线各管一段

**连带影响（同一次提交里一起改了）**

- `OperationAction` **26 → 29**（`CREATE_MUSIC` / `UPDATE_MUSIC` / `DELETE_MUSIC`）
- `AuditTarget` **9 → 10**（新增 `MUSIC`）
- 接口 **53 → 58**（1 公开 + 4 后台；`POST /upload` 是既有接口加参数，不新增接口）
- 权限矩阵算式 **`16 + 1 + 36 = 53` → `17 + 1 + 40 = 58`**
- 测试清单 **35 类 369 用例 → 36 类 403 用例**（新增 `MusicTest` 23；
  `UploadAdminTest` 12 → 22；`OperationLogTest` 18 → 19）
- README 的接口表、权限矩阵、审计动作表、测试清单、上传章节与配置样例同步改
- ⚠️ §21.4 里那串数字（`16 → 26`、`5 → 9`、`36 → 53`、`35 类 369 用例`）是 **F5 当时的快照**，
  加完音乐模块之后分别是 `26 → 29`、`9 → 10`、`53 → 58`、`36 类 403 用例`（本节就是新数字）

**前端待做（后端有意为之的契约，别改字段名）**

- `MusicVO` 字段：`id` / `title` / `artist` / `url` / `cover` / `lyrics` / `sort` / `status` / `createTime`
- **`lyrics` 是 LRC 原文文本**（不是文件路径）：前端自己解析时间戳（`app/utils/lrc.ts` 已经写好）；
  后端也**不**去读 mp3 内嵌的 `USLT` 标签 —— 歌词以数据库里那份为准
- `url` 既可能是 `/uploads/music/...`（上传回来的绝对地址），也可能是 http(s) 外链，
  前端不要对它有"必须是自己域名"的假设
- 列表**不分页**、`sort` 相同的按 `id` 升序（**顺序由后端保证，前端不要再排一遍** ——
  自己按 sort 重排会把第二段稳定排序弄丢）
- 删除仍然是**逻辑删除**（数据库行保留、`DELETE /admin/music/{id}` 的行为与 F5 四个模块同形）。
  ⚠️ **2026-09 变更：磁盘上的 mp3 现在会被清掉** —— 删歌时先查三处引用
  （别的曲目 / 文章正文与封面 / 文章附件行），都为零才删文件；`url` 是 http(s)
  外链时一个字节都不碰。所以"删完这首之后文件还在"只在"还有别处在用"时才成立，
  不要再把它当成固定的预期行为（详见 `MusicServiceImpl.delete` 与 README「音乐」一节）。
