...
>   而且不用配第二个阿里云服务。
>   改后的做法：把这两个大文件从 Nuxt 的构建产物里挪出来，
>   放进 Nginx 的静态目录（或挂给 Nginx 的容器），由 Nginx 直接返回 ——
>   好处是 `npm run build` 的体积立刻降下来，用户也不用等 Nuxt 进程转发大文件。
...
| **w3.2** | `将背景视频与音频移出构建产物，改由Nginx直接托管` | 12.4 MB 视频不再进构建产物（对应 E5）。**方案已调整：不上 OSS，放服务器本地由 Nginx 托管**，理由见上面那段说明 | `npm run build` 产物体积明显下降；Nginx 能直接返回该文件 |
