...
public interface ArticleMapper extends BaseMapper<Article> {
     ...
     *   （和上面那条 SUM(view_count) 一样）。
     *
     *   【调用频次 —— 更正一处不准确的旧说明】
     *   这里原来写着"它只在删文章时执行"，不准确。实际有两个入口：
     *     · 删文章（ArticleServiceImpl.remove）
     *     · 【编辑文章时移除附件】（replaceAttachments）
     *   而每次进入本方法都会连着查另外两张表（article_attachment / music），
     *   所以"一次编辑移除 K 个附件"= 3K 次全表扫描，且全都发生在写事务里。
     *
     *   【为什么核实过频次之后，结论仍然是"不改"】
     *     算最坏情况：1000 篇文章、一次移除 20 个附件 = 60 次 × 1000 行 = 6 万行扫描，
     *     毫秒级 —— 相对这次编辑本身要做的写库、标签重写、文件清理可以忽略。
     *     ⚠️ 特别记下一个【看着像优化、其实不是】的改法：把 K 个 key 用 UNION ALL
     *       合成一条 SQL。它减少的是网络往返，扫描次数一次都不会少。
     *       真要优化必须做成"一条 SQL 里对每个 key 出一列聚合"（动态列 + 结果映射），
     *       复杂度远超收益 —— 不做。
     *     唯一会推翻这个结论的前提是文章数到十万级：那时正确的做法是把引用关系
     *     落一张可索引的引用表，而不是继续在正文里 LOCATE。
     *
     *   个人博客的文章数是几百到几千 —— 见 README「为什么没有给 title 建索引」
     *   里同一条"前置通配符用不上索引"的说明。
     *
