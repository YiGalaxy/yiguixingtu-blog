...
public enum AuditTarget {
     ...
     * 音乐。
     *
     * 【又一次验证了这个枚举的设计目的】加音乐模块时这里只多一行、
     * action 那边按既有命名加三个前缀，没有动任何已有的代码与数据
     * （类注释开头那条预判，到这边已经是第四次兑现）。
     *
     * 【它的 target_id 是"某一条曲目的 id"】按 target_type = 'MUSIC' + target_id 查，
     * 就能拿到"这首歌被改过几次、什么时候被删的"——
     * 和 ABOUT 那种"恒为 1"的用法形成对照。
     */
    MUSIC,
     ...
     * 站点设置。
     *
     * 【它和 ABOUT 是同一族：target_id 恒为 1】
     *   因为站点设置也只有一行。所以按 target_type = 'SETTING' 查
     *   就能直接得到"站点设置被谁改过几次"的完整历史（连 id 条件都不用加）——
     *   这正是把"操作对象类型"单独拆成枚举的价值：加这一类只是多一行，
     *   不用动任何已有代码与数据（类注释开头那条预判的第五次兑现）。
     */
    SETTING
}
