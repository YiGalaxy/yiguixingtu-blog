...
public enum ResultCode {
    ...
    /**
     * 音乐不存在（含"已被逻辑删除"）。
     * 【注意它和"音频文件不存在"不是一回事】这个码只表示"库里没有这条音乐记录"；
     * 音频文件本身在磁盘上（或外链那边）由播放器自己去取，取不到是前端的兜底逻辑，
     * 和这条记录在不在没有关系。
     */
    MUSIC_NOT_FOUND(404, "音乐不存在"),
    ...
    /**
     * 站点把评论总开关关掉了（站点设置里那一项）。
     *
     * 【为什么要有这个码，而不是复用 PARAM_ERROR】
     *   用户提交的内容本身没有任何问题 —— 是【站点此刻不接受评论】。
     *   复用 PARAM_ERROR 会让前端把提示说成"你填的不对"，引导用户去改内容，
     *   而他要做的其实是"过一段时间再来"或者"算了"。
     *
     * 【为什么是 403】本项目里 403 这一档的语义是"请求本身没问题、
     *   但当前状态下不允许做这件事"（ACCOUNT_DISABLED 也是 403）。
     *   注意它仍然是【HTTP 200 + body.code】的业务错误，不是 HTTP 403 ——
     *   本项目的业务错误统一走 HTTP 200，见 README「统一返回与错误处理」。
     */
    COMMENT_DISABLED(403, "评论已关闭，暂时不能发表新的评论");
