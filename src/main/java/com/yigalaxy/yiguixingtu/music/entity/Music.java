 ...
 *
 * 【⚠️ 删掉一条音乐会留下"孤儿文件"—— 这是刻意的，写在这里免得被当成 bug】
 *   逻辑删除只标记数据库行，不会去删磁盘上那个 mp3。理由：
 *   ① 同一个文件可能被多条记录引用（站长先建一条、之后又建一条用同一个音源）
 *   ② 删文件是不可逆的，而"库里没引用了"这件事一旦判断错就再也找不回来
 *   ③ 这类文件的清理属于运维动作（按年月目录批量归档），不该藏在一次 DELETE 里
 *   所以：磁盘上的 uploads/music/ 会随着删除慢慢积累，需要时手工清理。
 *
